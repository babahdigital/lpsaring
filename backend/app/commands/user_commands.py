...
if TYPE_CHECKING:
    ...
    try:
        ...
        print(
            "CRITICAL ERROR: Could not import User/DailyUsageLog/PackageProfile models in user_commands.py. CLI functions may be limited."
        )
...
if MODELS_AVAILABLE:
    ROLE_INPUT_MAP_ADMIN_CREATE = {"1": UserRole.SUPER_ADMIN, "2": UserRole.ADMIN, "3": UserRole.USER}
    ROLE_DESCRIPTIONS_ADMIN_CREATE = [
        f"{num}: {ROLE_INPUT_MAP_ADMIN_CREATE[num].value}" for num in sorted(ROLE_INPUT_MAP_ADMIN_CREATE.keys())
    ]
else:
...
@click.option(
    "--blok",
    default="",
    prompt=f"Blok Rumah ({', '.join(b.value for b in UserBlok)}) (Opsional untuk Admin, Wajib untuk User)"
    if MODELS_AVAILABLE and UserBlok
    else "Blok Rumah (A-F) (Opsional untuk Admin, Wajib untuk User)",
    ...
    type=click.Choice(
        [b.value for b in UserBlok] + [""] if MODELS_AVAILABLE and UserBlok else ["A", "B", "C", "D", "E", "F", ""],
...
@click.option(
    "--kamar",
    default="",
    prompt=f"Nomor Kamar ({', '.join(k.value for k in UserKamar)}) (Opsional untuk Admin, Wajib untuk User)"
    if MODELS_AVAILABLE and UserKamar
    else "Nomor Kamar (Kamar_1-Kamar_6) (Opsional untuk Admin, Wajib untuk User)",
    ...
    type=click.Choice(
        [k.value for k in UserKamar] + [""]
        if MODELS_AVAILABLE and UserKamar
        else ["Kamar_1", "Kamar_2", "Kamar_3", "Kamar_4", "Kamar_5", "Kamar_6", ""],
        case_sensitive=False,
    ),
    show_default=False,
    show_choices=False,
...
@click.option(
    "--mikrotik-password",
    default=None,
    help="Password Mikrotik khusus (opsional, hanya jika role USER, jika kosong akan digenerate).",
)
@click.option(
    "--portal-password",
    required=False,
    help="Password Portal (hanya untuk Admin/Super Admin. Jika kosong, akan digenerate).",
)
@with_appcontext
def create_admin(phone, name, role, blok, kamar, mikrotik_password, portal_password):
    if not MODELS_AVAILABLE or not hasattr(db, "session") or not User or not ApprovalStatus or not UserRole:
        click.echo(
            click.style(
                "ERROR: Model User, ApprovalStatus, UserRole atau database session tidak dapat dimuat. Perintah create-admin tidak bisa dijalankan.",
                fg="red",
            )
        )
        return
    ...
    if not generate_password_hash_func:
        click.echo(
            click.style(
                "ERROR: Fungsi hashing password tidak tersedia. Perintah create-admin tidak bisa dijalankan.", fg="red"
            )
        )
        return
    ...
    try:
        ...
        if role_enum == UserRole.USER:
            ...
            if MODELS_AVAILABLE and UserBlok and blok_clean in [b.value for b in UserBlok]:
                ...
                click.echo(
                    click.style(
                        f"ERROR: Nilai Blok '{blok}' tidak valid untuk peran USER. Pilihan: {[b.value for b in UserBlok]}",
                        fg="red",
                    )
                )
                return
            ...
            if kamar_clean.isdigit() and 1 <= int(kamar_clean) <= 6:
                ...
                click.echo(
                    click.style(
                        f"ERROR: Nilai Kamar '{kamar}' tidak valid untuk peran USER. Pilihan: {[k.value for k in UserKamar]} atau angka 1-6.",
                        fg="red",
                    )
                )
                return
            ...
            if blok_clean and MODELS_AVAILABLE and UserBlok and blok_clean in [b.value for b in UserBlok]:
                ...
                click.echo(
                    click.style(
                        f"WARNING: Nilai Blok '{blok}' tidak valid untuk Admin. Dikosongkan. Pilihan: {[b.value for b in UserBlok]}",
                        fg="yellow",
                    )
                )
                blok_val_for_db = None
            ...
            if kamar_clean:
                if kamar_clean.isdigit() and 1 <= int(kamar_clean) <= 6:
                    ...
                    click.echo(
                        click.style(
                            f"WARNING: Nilai Kamar '{kamar}' tidak valid untuk Admin. Dikosongkan. Pilihan: {[k.value for k in UserKamar]} atau angka 1-6.",
                            fg="yellow",
                        )
                    )
                    kamar_val_for_db = None
    ...
    if role_enum == UserRole.USER:
        ...
        click.echo(
            click.style("INFO: Password portal otomatis digenerate (6 digit angka) untuk peran ADMIN.", fg="blue")
        )
        ...
        if portal_password:
            ...
            click.echo(
                click.style(
                    "INFO: Password portal otomatis digenerate (6 digit angka) untuk peran SUPER_ADMIN.", fg="blue"
                )
            )
    else:
    ...
    if role_enum == UserRole.USER:
        final_mikrotik_password_for_db = (
            mikrotik_password if mikrotik_password else generate_random_password(length=8, type="numeric")
        )
                ...
                if mikrotik_api_conn:
                    ...
                    if current_app.config.get("MIKROTIK_SEND_SESSION_TIMEOUT", False):
                        if new_user.quota_expiry_date:
                            time_remaining = (
                                new_user.quota_expiry_date - datetime.now(dt_timezone.utc)
                            ).total_seconds()
                    ...
                    if activate_success:
                        click.echo(
                            click.style(
                                f"INFO: Akun Mikrotik untuk user '{new_user.full_name}' berhasil dibuat/diupdate. Mikrotik: {mikrotik_msg}",
                                fg="blue",
                            )
                        )
                    else:
                        click.echo(
                            click.style(
                                f"WARNING: Gagal membuat/mengupdate akun Mikrotik untuk user '{new_user.full_name}': {mikrotik_msg}",
                                fg="yellow",
                            )
                        )
                else:
                    click.echo(
                        click.style("WARNING: Gagal mendapatkan koneksi Mikrotik untuk pembuatan user.", fg="yellow")
                    )
        except Exception as e_mt:
            current_app.logger.error(
                f"ERROR: Exception saat membuat akun Mikrotik untuk user '{new_user.full_name}': {str(e_mt)}",
                exc_info=True,
            )
            click.echo(
                click.style(
    ...
    try:
        ...
        click.echo(
            click.style(
                f"SUKSES: Pengguna {role_label} '{new_user.full_name}' (ID: {new_user.id}) berhasil dibuat.", fg="green"
            )
        )
        ...
        if (
            WHATSAPP_AVAILABLE
            and NOTIFICATION_SERVICE_AVAILABLE
            and settings_service.get_setting("ENABLE_WHATSAPP_NOTIFICATIONS", "False") == "True"
        ):
            message_body = ""
            if new_user.role == UserRole.ADMIN or new_user.role == UserRole.SUPER_ADMIN:
                context = {
                    ...
                    "password": password_portal_plain_text,
                }
...
@user_cli_bp.command(
    "create-superadmin", help="Membuat pengguna Super Admin baru dengan password otomatis (dikirim WA)."
)
@click.option(
    "--phone",
    required=True,
    prompt="Nomor Telepon Super Admin (format 08... atau +62...)",
    help="Nomor telepon unik Super Admin.",
)
@click.option("--name", required=True, prompt="Nama Lengkap Super Admin", help="Nama lengkap Super Admin.")
@click.option(
    "--blok",
    default="",
    prompt=f"Blok Rumah ({', '.join(b.value for b in UserBlok)}) (Opsional)"
    if MODELS_AVAILABLE and UserBlok
    else "Blok Rumah (A-F) (Opsional)",
...
@click.option(
    "--kamar",
    default="",
    prompt=f"Nomor Kamar ({', '.join(k.value for k in UserKamar)}) (Opsional)"
    if MODELS_AVAILABLE and UserKamar
    else "Nomor Kamar (Kamar_1-Kamar_6) (Opsional)",
...
def create_superadmin(phone, name, blok, kamar):
    if not MODELS_AVAILABLE or not hasattr(db, "session") or not User or not ApprovalStatus or not UserRole:
        click.echo(
            click.style(
                "ERROR: Model User, ApprovalStatus, UserRole atau database session tidak dapat dimuat. Perintah create-superadmin tidak bisa dijalankan.",
                fg="red",
            )
        )
        return
    if not generate_password_hash_func:
        click.echo(
            click.style(
                "ERROR: Fungsi hashing password tidak tersedia. Perintah create-superadmin tidak bisa dijalankan.",
                fg="red",
            )
        )
        return
    ...
    try:
        ...
        blok_val_for_db = (
            blok_clean if MODELS_AVAILABLE and UserBlok and blok_clean in [b.value for b in UserBlok] else None
        )
    ...
    try:
        ...
        click.echo(
            click.style(
                f"SUKSES: Pengguna Super Admin '{new_user.full_name}' (ID: {new_user.id}) berhasil dibuat.", fg="green"
            )
        )
        ...
        if (
            ...
            if message_body:
                ...
                click.echo(
                    click.style(
                        f"INFO: Notifikasi WhatsApp berisi password dikirim ke {new_user.phone_number}.", fg="blue"
                    )
                )
            else:
                current_app.logger.warning(
                    "Gagal mendapatkan pesan notifikasi WhatsApp. Password tidak terkirim via WA."
                )
                click.echo(
                    click.style(
                        "WARNING: Gagal mendapatkan pesan notifikasi WhatsApp. Password tidak terkirim via WA.",
                        fg="yellow",
                    )
                )
                click.echo(
                    click.style(f"  PASSWORD SUPER ADMIN: {password_portal_plain_text}", fg="magenta", bold=True)
                )
        else:
            current_app.logger.info(
                "Notifikasi WhatsApp dinonaktifkan di pengaturan atau klien/service tidak tersedia."
            )
            click.echo(
                click.style(
                    "INFO: Notifikasi WhatsApp dinonaktifkan di pengaturan atau klien/service tidak tersedia. Password tidak terkirim via WA.",
                    fg="yellow",
                )
            )
...
@click.option(
    "--kamar", default=None, help="Kamar baru (Kamar_1/Kamar_2/...) atau 'kosong' untuk menghapus. Wajib untuk User."
)
...
def update_user(phone_number, nama, blok, kamar, aktif, admin_id, is_unlimited, expiry_date):
    if not MODELS_AVAILABLE or not hasattr(db, "session") or not User or not UserBlok or not UserKamar or not UserRole:
        click.echo(
            click.style(
                "ERROR: Model User, UserBlok, UserKamar, UserRole atau database session tidak dapat dimuat.", fg="red"
            )
        )
        return
    ...
    if admin_id:
        try:
            ...
            if fetched_admin and fetched_admin.is_admin_role:
                ...
                click.echo(
                    click.style(
                        f"WARNING: Pengguna '{fetched_admin.full_name}' bukan Admin (Role: {fetched_admin.role.value}).",
                        fg="yellow",
                    )
                )
            else:
            ...
            current_app.logger.warning(
                f"Tidak dapat verifikasi role admin untuk admin_id '{admin_id}': {e}", exc_info=True
            )
            click.echo(
                click.style(f"WARNING: Tidak dapat verifikasi role admin untuk admin_id '{admin_id}'.", fg="yellow")
            )
    ...
    if blok is not None:
        if blok.lower() == "kosong":
            ...
            if MODELS_AVAILABLE and UserBlok and blok.upper() in [b.value for b in UserBlok]:
                ...
                click.echo(
                    click.style(
                        f"ERROR: Nilai blok '{blok}' tidak valid. Pilihan: {[b.value for b in UserBlok]}", fg="red"
                    )
                )
                return
    ...
    if kamar is not None:
        if kamar.lower() == "kosong":
            ...
            if kamar.isdigit() and 1 <= int(kamar) <= 6:
                ...
                click.echo(
                    click.style(
                        f"ERROR: Nilai kamar '{kamar}' tidak valid. Pilihan: {[k.value for k in UserKamar]} atau angka 1-6.",
                        fg="red",
                    )
                )
                return
    ...
    if is_unlimited is not None:
        if user_to_update.is_unlimited_user != is_unlimited:
            ...
            click.echo(
                click.style(f"INFO: Status unlimited pengguna sudah '{'Ya' if is_unlimited else 'Tidak'}'.", fg="blue")
            )
        ...
        if expiry_date.lower() == "kosong":
            ...
            try:
                ...
                if user_to_update.quota_expiry_date != parsed_date:
                    ...
                    click.echo(
                        click.style(f"INFO: Tanggal kadaluarsa kuota pengguna sudah '{expiry_date}'.", fg="blue")
                    )
            except ValueError:
                click.echo(
                    click.style(
                        f"ERROR: Format tanggal kadaluarsa '{expiry_date}' tidak valid. Gunakan YYYY-MM-DD.", fg="red"
                    )
                )
                return
    ...
    try:
        ...
        click.echo(
            click.style(
                f"SUKSES: Data pengguna '{user_to_update.full_name}' ({normalized_phone}) berhasil diperbarui.",
                fg="green",
            )
        )
...
@click.option(
    ...
    "-s",
    type=click.Choice(
        [s.value for s in ApprovalStatus] if MODELS_AVAILABLE and ApprovalStatus else [], case_sensitive=False
    ),
    default=None,
    help="Filter status approval.",
)
@click.option(
    "--role",
    "-r",
    type=click.Choice([r.value for r in UserRole] if MODELS_AVAILABLE and UserRole else [], case_sensitive=False),
    default=None,
    help="Filter role.",
)
...
def list_users(status, role, active, limit, search):
    if not MODELS_AVAILABLE or not hasattr(db, "session") or not User or not ApprovalStatus or not UserRole:
        click.echo(
            click.style(
                "ERROR: Model User, ApprovalStatus, UserRole atau database session tidak dapat dimuat.", fg="red"
            )
        )
        return
    ...
    for user_obj in users_to_list:
        ...
        user_line = "{:<37} {:<15} {:<20} {:<7} {:<7} {:<18} {:<13} {:<7} {:<15} {:<15} {:<10} {:<18}".format(
            ...
            user_obj.approval_status.value
            if isinstance(user_obj.approval_status, enum.Enum)
            else str(user_obj.approval_status),
...
# --- Perintah approve ---
@user_cli_bp.command(
    "approve", help="Menyetujui pengguna PENDING atau sinkronisasi ulang pengguna APPROVED ke Mikrotik."
)
@click.argument("phone_number", type=str)
@click.option(
    "--admin-id", "-a", type=str, default=None, help="UUID Admin yang melakukan aksi (opsional, untuk pencatatan)."
)
@click.option(
    "--force-sync",
    "-fs",
    is_flag=True,
    help="Paksa sinkronisasi (generate password Mikrotik baru & update ke Mikrotik) meskipun pengguna sudah APPROVED.",
)
@with_appcontext
def approve_user(phone_number, admin_id, force_sync):
    if not MODELS_AVAILABLE or not hasattr(db, "session") or not User or not ApprovalStatus or not UserRole:
        click.echo(
            click.style("ERROR: Model User/ApprovalStatus/UserRole atau database session tidak dapat dimuat.", fg="red")
        )
        return
    if not MIKROTIK_CLIENT_AVAILABLE or not WHATSAPP_AVAILABLE or not NOTIFICATION_SERVICE_AVAILABLE:
        current_app.logger.error(
            "Klien Mikrotik, WhatsApp, atau Notification Service tidak termuat. Perintah tidak bisa dijalankan."
        )
        click.echo(
            click.style(
                "ERROR: Klien Mikrotik, WhatsApp, atau Notification Service tidak termuat. Perintah tidak bisa dijalankan.",
                fg="red",
            )
        )
        return
    ...
    if admin_id:
        try:
            ...
            if fetched_admin and fetched_admin.is_admin_role:
                ...
                current_app.logger.warning(
                    f"Pengguna '{fetched_admin.full_name}' bukan Admin (Role: {fetched_admin.role.value})."
                )
                click.echo(
                    click.style(
                        f"WARNING: Pengguna '{fetched_admin.full_name}' bukan Admin (Role: {fetched_admin.role.value}).",
    ...
    if not is_initial_approval and user_to_process.approval_status == ApprovalStatus.APPROVED and not force_sync:
        click.echo(
            click.style(
                f"INFO: Pengguna '{user_to_process.full_name}' sudah {user_to_process.approval_status.value}.",
                fg="yellow",
            )
        )
    ...
    if force_sync:
        ...
        click.echo(
            click.style(
                f"  [INFO] Force Sync: Password Mikrotik baru '{password_untuk_mikrotik}' akan dibuat.", fg="blue"
            )
        )
    elif is_initial_approval:
        if (
            user_to_process.mikrotik_password
            and len(user_to_process.mikrotik_password) >= 6
            and user_to_process.mikrotik_password.isdigit()
        ):
            password_untuk_mikrotik = user_to_process.mikrotik_password
            click.echo(
                click.style(
                    f"  [INFO] Initial Approval: Menggunakan password yang sudah ada: '{password_untuk_mikrotik}'.",
                    fg="blue",
                )
            )
        else:
            ...
            click.echo(
                click.style(
                    f"  [INFO] Initial Approval: Password Mikrotik baru '{password_untuk_mikrotik}' dibuat (sebelumnya kosong/tidak valid).",
                    fg="blue",
                )
            )
    else:
    ...
    try:
            ...
            if user_to_process.role == UserRole.USER:
                if user_to_process.is_tamping:
                    if not user_to_process.tamping_type:
                        current_app.logger.error(
                            f"Pengguna '{user_to_process.full_name}' (role USER tamping) tidak memiliki jenis tamping. Penyetujuan dibatalkan."
                        )
                        click.echo(
                            click.style(
                                f"  [DB] ERROR: Pengguna '{user_to_process.full_name}' (role USER tamping) tidak memiliki jenis tamping. Penyetujuan dibatalkan.",
                                fg="red",
                            )
                        )
                        return
                    ...
                    if not user_to_process.blok:
                        current_app.logger.error(
                            f"Pengguna '{user_to_process.full_name}' (role USER) tidak memiliki data Blok. Penyetujuan dibatalkan."
                        )
                        click.echo(
                            click.style(
                                f"  [DB] ERROR: Pengguna '{user_to_process.full_name}' (role USER) tidak memiliki data Blok. Penyetujuan dibatalkan.",
                                fg="red",
                            )
                        )
                        return
                    if not user_to_process.kamar:
                        current_app.logger.error(
                            f"Pengguna '{user_to_process.full_name}' (role USER) tidak memiliki data Kamar. Penyetujuan dibatalkan."
                        )
                        click.echo(
                            click.style(
                                f"  [DB] ERROR: Pengguna '{user_to_process.full_name}' (role USER) tidak memiliki data Kamar. Penyetujuan dibatalkan.",
                                fg="red",
                            )
                        )
                        return
        ...
        password_log_info_db = (
            f" Password Mikrotik diupdate ke '{password_untuk_mikrotik}'."
            if password_generated_for_this_action or force_sync
            else " Password Mikrotik tidak berubah."
        )
        current_app.logger.info(f"Status pengguna diperbarui{admin_log_info_db}.{password_log_info_db}")
        click.echo(
            click.style(
                f"  [DB] SUKSES: Status pengguna diperbarui{admin_log_info_db}.{password_log_info_db}", fg="green"
            )
        )
    except Exception as e_db_approve:
        db.session.rollback()
        current_app.logger.error(
            f"Error DB {action_description} untuk {normalized_phone}: {str(e_db_approve)}", exc_info=True
        )
    ...
    if MIKROTIK_CLIENT_AVAILABLE:
        ...
        if not mikrotik_username_mt:
            current_app.logger.warning(
                f"Tidak dapat konversi nomor ke username Mikrotik. Update Mikrotik dilewati untuk {user_to_process.id}."
            )
            click.echo(
                click.style(
                    "  [Mikrotik] WARNING: Tidak dapat konversi nomor ke username Mikrotik. Update Mikrotik dilewati.",
                    fg="yellow",
                )
            )
            ...
            try:
                with get_mikrotik_connection() as mikrotik_api_conn:
                    if mikrotik_api_conn:
                        current_app.logger.info(
                            f"Mencoba update Mikrotik untuk '{mikrotik_username_mt}' (ID DB: {user_to_process.id}) profile: {mikrotik_profile_to_use} dengan password '{password_untuk_mikrotik}'"
                        )
                        click.echo(
                            f"  [Mikrotik] INFO: Mencoba update Mikrotik untuk '{mikrotik_username_mt}' (ID DB: {user_to_process.id}) profile: {mikrotik_profile_to_use} dengan password '{password_untuk_mikrotik}'"
                        )
                        ...
                        if current_app.config.get("MIKROTIK_SEND_LIMIT_BYTES_TOTAL", False):
                            limit_bytes_total_val = int(
                                max(
                                    0,
                                    user_to_process.total_quota_purchased_mb
                                    - (user_to_process.total_quota_used_mb or 0),
                                )
                                * 1024
                                * 1024
                            )
                        ...
                        if current_app.config.get("MIKROTIK_SEND_SESSION_TIMEOUT", False):
                            ...
                            if user_to_process.quota_expiry_date:
                                time_remaining = (
                                    user_to_process.quota_expiry_date - datetime.now(dt_timezone.utc)
                                ).total_seconds()
                        ...
                        if mikrotik_update_success:
                            current_app.logger.info(
                                f"Mikrotik update SUKSES untuk {mikrotik_username_mt}: {mikrotik_message_detail}"
                            )
                            click.echo(click.style(f"  [Mikrotik] SUKSES: {mikrotik_message_detail}", fg="green"))
                        else:
                            current_app.logger.error(
                                f"Mikrotik update GAGAL untuk {mikrotik_username_mt}: {mikrotik_message_detail}"
                            )
                ...
                current_app.logger.error(
                    f"Exception Mikrotik untuk {mikrotik_username_mt}: {str(e_mt_approve)}", exc_info=True
                )
    ...
    if (
        WHATSAPP_AVAILABLE
        and NOTIFICATION_SERVICE_AVAILABLE
        and user_to_process.phone_number
        and settings_service.get_setting("ENABLE_WHATSAPP_NOTIFICATIONS", "False") == "True"
        and (is_initial_approval or (force_sync and password_generated_for_this_action))
    ):
        try:
            username_display_for_wa = (
                format_to_local_phone(user_to_process.phone_number) or user_to_process.phone_number
            )
            ...
            if message_body_wa:
                ...
                if whatsapp_notification_success:
                    ...
                    click.echo(
                        click.style(
                            f"  [WhatsApp] SUKSES: Notifikasi & password dikirim ke {user_to_process.phone_number}.",
                            fg="green",
                        )
                    )
                else:
                    ...
                    click.echo(
                        click.style(
                            f"  [WhatsApp] ERROR: Gagal kirim notifikasi ke {user_to_process.phone_number}.", fg="red"
                        )
                    )
        except Exception as e_wa_approve:
            current_app.logger.error(
                f"Exception WA untuk {user_to_process.phone_number}: {str(e_wa_approve)}", exc_info=True
            )
        ...
        if is_initial_approval or (force_sync and password_generated_for_this_action):
            click.echo(
                click.style(
                    f"    -> Info Admin: Password Mikrotik {user_to_process.full_name}: {password_untuk_mikrotik}",
                    fg="blue",
                )
            )
        ...
        click.echo(
            click.style("  [WhatsApp] INFO: Notification Service tidak tersedia, notifikasi dilewati.", fg="yellow")
        )
        ...
        click.echo(
            click.style(
                "  [WhatsApp] INFO: Notifikasi WhatsApp dinonaktifkan di pengaturan, notifikasi dilewati.", fg="yellow"
            )
        )
    ...
    if not mikrotik_update_success and MIKROTIK_CLIENT_AVAILABLE and (is_initial_approval or force_sync):
        ...
        click.echo(
            click.style(
                f"     Password Mikrotik yang seharusnya digunakan: {password_untuk_mikrotik}. Perlu tindakan manual.",
                fg="yellow",
            )
        )
    if (
        not whatsapp_notification_success
        ...
        click.echo(
            click.style(
                f"  -> PENTING: Notifikasi WhatsApp GAGAL. Pastikan pengguna tahu password baru: {password_untuk_mikrotik}",
                fg="yellow",
            )
        )
...
@click.argument("phone_number", type=str)
@click.option(
    "--admin-id", "-a", type=str, default=None, help="UUID Admin yang melakukan penolakan (opsional, untuk log)."
)
@click.option(
    "--reason",
    "-msg",
    type=str,
    default="Pendaftaran Anda tidak dapat kami setujui saat ini.",
    help="Pesan alasan penolakan untuk dikirim ke pengguna.",
)
...
def reject_user(phone_number, admin_id, reason, force):
    ...
    if not MIKROTIK_CLIENT_AVAILABLE or not NOTIFICATION_SERVICE_AVAILABLE:
        current_app.logger.error(
            "Klien Mikrotik atau Notification Service tidak termuat. Perintah tidak bisa dijalankan."
        )
        click.echo(
            click.style(
                "ERROR: Klien Mikrotik atau Notification Service tidak termuat. Perintah tidak bisa dijalankan.",
                fg="red",
            )
        )
        return
    ...
    if user_to_reject.approval_status != ApprovalStatus.PENDING_APPROVAL:
        current_status_val = (
            user_to_reject.approval_status.value
            if isinstance(user_to_reject.approval_status, enum.Enum)
            else str(user_to_reject.approval_status)
        )
        click.echo(
            click.style(
                f"INFO: Pengguna '{user_to_reject.full_name}' tidak PENDING (Status: {current_status_val}).",
                fg="yellow",
            )
        )
    ...
    if not force:
        ...
        click.echo(
            f"  Nama: {user_to_reject.full_name}, Telp: {user_to_reject.phone_number}, Status: {user_to_reject.approval_status.value}"
        )
        click.confirm(
        ...
        try:
            with get_mikrotik_connection() as mikrotik_api_conn:
                if mikrotik_api_conn:
                    ...
                    success_profile, msg_profile = set_hotspot_user_profile(
                        mikrotik_api_conn, mikrotik_username_target, expired_profile_name
                    )
            ...
            current_app.logger.error(
                f"Exception Mikrotik pindah profil untuk {user_to_reject.id} saat penolakan: {str(e_mt)}", exc_info=True
            )
        ...
        try:
            ...
            if message_body_reject:
                ...
                if whatsapp_notification_sent:
                    ...
                    click.echo(
                        click.style(
                            f"  [WhatsApp] INFO: Notifikasi penolakan dikirim ke {user_phone_for_notif}.", fg="blue"
                        )
                    )
                else:
                    ...
                    click.echo(
                        click.style(
                            f"  [WhatsApp] ERROR: Gagal kirim notifikasi penolakan ke {user_phone_for_notif}.", fg="red"
                        )
                    )
        except Exception as e_wa_reject_cli:
            current_app.logger.error(
                f"Exception WA penolakan ke {user_phone_for_notif}: {str(e_wa_reject_cli)}", exc_info=True
            )
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"User REJECTED and DELETED: ID={user_id_for_log}, Phone={user_phone_for_notif}, Name='{user_full_name_for_log}'. Mikrotik: attempted={mikrotik_status_action}. Admin ID: {admin_id_for_log if admin_performing_rejection else 'System'}."
        )
        click.echo(
            click.style(
                f"\nSUKSES: Pengguna '{user_full_name_for_log}' ({user_phone_for_notif}) DITOLAK & DIHAPUS dari database.",
                fg="green",
                bold=True,
            )
        )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error reject/delete user {user_phone_for_notif} from DB: {str(e_db_reject)}", exc_info=True
        )
...
def delete_user(phone_number, admin_id, force):
    ...
    if not force:
        ...
        click.echo(
            f"  Role: {user_to_delete.role.value}, Status: {user_to_delete.approval_status.value}, Aktif: {'Ya' if user_to_delete.is_active else 'Tidak'}"
        )
        ...
        click.confirm(
            click.style(
                "\nAnda YAKIN MENGHAPUS PERMANEN pengguna ini dari DB dan Mikrotik?",
                fg="red",
                underline=True,
                bold=True,
            ),
    ...
    if MIKROTIK_CLIENT_AVAILABLE and mikrotik_username_to_delete:
        if user_to_delete.mikrotik_password:
            ...
            try:
                with get_mikrotik_connection() as mikrotik_api_conn:
                    if mikrotik_api_conn:
                        mikrotik_deletion_successful, mikrotik_deletion_message = delete_hotspot_user(
                            mikrotik_api_conn, mikrotik_username_to_delete
                        )
                        if mikrotik_deletion_successful:
                            ...
                            current_app.logger.warning(
                                f"Mikrotik hapus GAGAL/tidak ditemukan. Pesan: {mikrotik_deletion_message}"
                            )
                            click.echo(
                                click.style(
                                    f"  [Mikrotik] WARNING: Gagal hapus/tidak ditemukan. Pesan: {mikrotik_deletion_message}",
                                    fg="yellow",
                                )
                            )
                    else:
                ...
                current_app.logger.error(
                    f"Exception hapus user '{mikrotik_username_to_delete}' dari Mikrotik: {str(e_mt_delete_cli)}",
                    exc_info=True,
                )
            ...
            mikrotik_deletion_message = (
                "Pengguna tidak memiliki password Mikrotik (tidak aktif di Mikrotik), penghapusan dilewati."
            )
    ...
    try:
        ...
        current_app.logger.info(
            f"User DELETED from DB: ID={user_id_log}, Phone={user_phone_log}, Name='{user_name_log}'. Mikrotik: attempted={mikrotik_deletion_attempted}, success={mikrotik_deletion_successful} ('{mikrotik_deletion_message}'). Admin ID: {admin_id_for_log_del if admin_performing_deletion else 'System/CLI'}."
        )
        click.echo(
            click.style(
    ...
    if mikrotik_deletion_attempted:
        if mikrotik_deletion_successful:
            ...
            click.echo(
                click.style(f"  - Mikrotik: GAGAL/TIDAK DIKONFIRMASI. Pesan: {mikrotik_deletion_message}", fg="yellow")
            )
...
def set_user_role(phone_number, new_role, admin_id):
    ...
    user_to_update_role = db.session.execute(
        db.select(User).filter_by(phone_number=normalized_phone)
    ).scalar_one_or_none()
    ...
    old_role_val = (
        user_to_update_role.role.value
        if isinstance(user_to_update_role.role, enum.Enum)
        else str(user_to_update_role.role)
    )
    new_role_val = role_enum_to_set.value

    if user_to_update_role.role == role_enum_to_set:
        click.echo(
            click.style(f"INFO: Pengguna '{user_to_update_role.full_name}' sudah role '{old_role_val}'.", fg="yellow")
        )
        return
    ...
    if old_role_val != UserRole.USER.value and new_role_val == UserRole.USER.value:
        click.echo(
            click.style(
                f"INFO: Downgrade dari {old_role_val} ke {new_role_val}. Memerlukan data Blok & Kamar.", fg="blue"
            )
        )
        if not user_to_update_role.blok or not user_to_update_role.kamar:
            current_app.logger.error(
                f"Pengguna '{user_to_update_role.full_name}' (role USER) tidak memiliki data Blok atau Kamar yang wajib. Upgrade/downgrade dibatalkan."
            )
            click.echo(
                click.style(
                    "ERROR: Pengguna tidak memiliki data Blok atau Kamar yang wajib untuk peran USER. Upgrade/downgrade dibatalkan.",
                    fg="red",
                )
            )
            click.echo(
                click.style(
                    "Harap update pengguna dengan Blok dan Kamar yang valid terlebih dahulu menggunakan 'flask user update'.",
                    fg="yellow",
                )
            )
            return
    ...
    if new_role_val == UserRole.ADMIN.value or new_role_val == UserRole.SUPER_ADMIN.value:
        ...
        click.echo(
            click.style(
                f"INFO: Password portal BARU (6 digit angka) telah di-generate untuk peran {new_role_val}. Password lama ditimpa.",
                fg="blue",
            )
        )
        ...
        click.echo(
            click.style(f"INFO: Password Mikrotik baru digenerate (8 digit angka) untuk {new_role_val}.", fg="blue")
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"User role updated: ID={user_to_update_role.id}, Phone={normalized_phone}, Name='{user_to_update_role.full_name}'. Old: {old_role_val}, New: {new_role_val}. Admin ID: {admin_id_for_log_sr if admin_performing_set_role else 'System/CLI'}."
        )
        click.echo(
            click.style(
                f"SUKSES: Role '{user_to_update_role.full_name}' ({normalized_phone}) diubah dari {old_role_val} ke {new_role_val}.",
                fg="green",
            )
        )
        ...
        if (
            WHATSAPP_AVAILABLE
            and NOTIFICATION_SERVICE_AVAILABLE
            and user_to_update_role.phone_number
            and settings_service.get_setting("ENABLE_WHATSAPP_NOTIFICATIONS", "False") == "True"
        ):
            phone_display_local = (
                format_to_local_phone(user_to_update_role.phone_number) or user_to_update_role.phone_number
            )
            ...
            if new_role_val == UserRole.USER.value and old_role_val != UserRole.USER.value:
                ...
                message_body_role_change = get_notification_message(
                    "user_downgrade_to_user_with_password", context_for_notif
                )
                    ...
                    if mikrotik_username_mt:
                        with get_mikrotik_connection() as api_conn:
                            if api_conn:
                                ...
                                if activate_success:
                                    current_app.logger.info(
                                        f"Akun Mikrotik {user_to_update_role.full_name} disinkronkan untuk peran USER."
                                    )
                                    click.echo(
                                        click.style(
                                            f"INFO: Akun Mikrotik {user_to_update_role.full_name} disinkronkan untuk peran USER.",
                                            fg="blue",
                                        )
                                    )
                                else:
                                    current_app.logger.warning(
                                        f"Gagal sinkronisasi Mikrotik saat downgrade role: {mikrotik_msg}"
                                    )
                                    click.echo(
                                        click.style(
                                            f"WARNING: Gagal sinkronisasi Mikrotik saat downgrade role: {mikrotik_msg}",
                                            fg="yellow",
                                        )
                                    )
                            else:
                                ...
                                click.echo(
                                    click.style("WARNING: Gagal koneksi Mikrotik saat downgrade role.", fg="yellow")
                                )
                    else:
                        current_app.logger.warning(
                            "Nomor telepon tidak dapat diformat ke username Mikrotik saat downgrade role."
                        )
                        click.echo(
                            click.style(
                                "WARNING: Nomor telepon tidak dapat diformat ke username Mikrotik saat downgrade role.",
                                fg="yellow",
                            )
                        )
                ...
                message_body_role_change = get_notification_message(
                    "user_upgrade_to_admin_with_password", context_for_notif
                )
                    ...
                    if mikrotik_username:
                        with get_mikrotik_connection() as api_conn:
                            if api_conn:
                                ...
                                if delete_success:
                                    current_app.logger.info(
                                        f"Akun Mikrotik {mikrotik_username} dihapus karena diupgrade ke {new_role_val}."
                                    )
                                    click.echo(
                                        click.style(
                                            f"INFO: Akun Mikrotik {mikrotik_username} dihapus karena diupgrade ke {new_role_val}.",
                                            fg="blue",
                                        )
                                    )
                                    ...
                                    current_app.logger.warning(
                                        f"Gagal menghapus akun Mikrotik {mikrotik_username} saat upgrade role: {delete_msg}"
                                    )
                                    click.echo(
                                        click.style(
                                            f"WARNING: Gagal menghapus akun Mikrotik {mikrotik_username} saat upgrade role: {delete_msg}",
                                            fg="yellow",
                                        )
                                    )
                            else:
                                ...
                                click.echo(
                                    click.style("WARNING: Gagal koneksi Mikrotik saat upgrade role.", fg="yellow")
                                )
                    else:
                        current_app.logger.warning(
                            "Nomor telepon tidak dapat diformat ke username Mikrotik saat upgrade role."
                        )
                        click.echo(
                            click.style(
                                "WARNING: Nomor telepon tidak dapat diformat ke username Mikrotik saat upgrade role.",
                                fg="yellow",
                            )
                        )

            if message_body_role_change:
                whatsapp_notification_sent = send_whatsapp_message(
                    user_to_update_role.phone_number, message_body_role_change
                )
                if whatsapp_notification_sent:
                    click.echo(
                        click.style(
                            f"INFO: Notifikasi WhatsApp perubahan role dikirim ke {user_to_update_role.phone_number}.",
                            fg="blue",
                        )
                    )
                else:
                    click.echo(
                        click.style(
                            f"WARNING: Gagal kirim notifikasi WhatsApp perubahan role ke {user_to_update_role.phone_number}.",
                            fg="yellow",
                        )
                    )
        ...
        if not whatsapp_notification_sent and (WHATSAPP_AVAILABLE and NOTIFICATION_SERVICE_AVAILABLE):
            current_app.logger.info(
                "Notifikasi WhatsApp perubahan role tidak dikirim (fitur notifikasi nonaktif/gagal)."
            )
            click.echo(
                click.style(
                    "INFO: Notifikasi WhatsApp perubahan role tidak dikirim (fitur notifikasi nonaktif/gagal).",
                    fg="yellow",
                )
            )
        elif not (WHATSAPP_AVAILABLE and NOTIFICATION_SERVICE_AVAILABLE):
            current_app.logger.info(
                "Notifikasi WhatsApp perubahan role tidak dikirim (klien WhatsApp/Notif Service tidak tersedia)."
            )
            click.echo(
                click.style(
                    "INFO: Notifikasi WhatsApp perubahan role tidak dikirim (klien WhatsApp/Notif Service tidak tersedia).",
                    fg="yellow",
                )
            )
        elif settings_service.get_setting("ENABLE_WHATSAPP_NOTIFICATIONS", "False") != "True":
            current_app.logger.info("Notifikasi WhatsApp dinonaktifkan di pengaturan.")
            click.echo(
                click.style("INFO: Notifikasi WhatsApp dinonaktifkan di pengaturan, notifikasi dilewati.", fg="yellow")
            )
...
@click.option(
    ...
    help="Password baru untuk pengguna. Akan di-hash sebelum disimpan.",
)
@with_appcontext
def set_user_password(phone_number, password):
    ...
    try:
        ...
        click.echo(
            click.style(
                f"SUKSES: Password untuk pengguna '{user_to_update.full_name}' ({normalized_phone}) telah berhasil diatur ulang.",
                fg="green",
            )
        )
    except Exception as e:
...
def notify_access_status(phone_number, status_key, reason, remaining_mb, remaining_percent, expiry_date):
    ...
    phone_variations = get_phone_number_variations(normalized_phone)
    user = db.session.execute(db.select(User).filter(User.phone_number.in_(phone_variations))).scalar_one_or_none()
...
def init_superadmin(phone, name, password):
    ...
    if not phone or not password:
        click.echo(
            click.style(
                "ERROR: phone/password wajib. Isi argumen atau env SUPERADMIN_PHONE & SUPERADMIN_PASSWORD.", fg="red"
            )
        )
        return
    if len(password) < 6:
    ...
    try:
        existing_users = (
            db.session.execute(db.select(User).filter(User.phone_number.in_(phone_variations))).scalars().all()
        )
        ...
        db.session.commit()
        click.echo(
            click.style(f"SUKSES: Super Admin '{name}' dibuat ulang dengan nomor {normalized_phone}.", fg="green")
        )
    except Exception as e:
        db.session.rollback()
...
@click.option(
    ...
    default=10240,
    show_default=True,
    help="Total kuota dibeli (MB). Default: 10GB.",
)
@click.option(
    "--days", "-d", type=int, default=7, show_default=True, help="Jumlah hari ke belakang untuk data pemakaian."
)
@click.option("--admin-id", "-a", type=str, default=None, help="UUID Admin yang melakukan aksi (opsional, untuk log).")
@with_appcontext
def seed_user_usage(phone_number, quota_purchased, days, admin_id):
    ...
    try:
        ...
        click.echo(
            click.style("ERROR: Model DailyUsageLog tidak terdefinisi dengan benar atau tidak dapat diakses.", fg="red")
        )
        return
    ...
    click.echo(
        click.style(f"Memulai pembuatan data dummy untuk '{user_to_seed.full_name}' ({normalized_phone})...", fg="cyan")
    )
    ...
    try:
        ...
        if num_deleted > 0:
            ...
            click.echo(
                click.style(f"  INFO: Menghapus {num_deleted} log pemakaian lama dalam rentang tanggal.", fg="blue")
            )
...
def add_mikrotik_profile(name, rate_limit, shared_users, description):
    ...
    if not MIKROTIK_CLIENT_AVAILABLE:
        ...
        click.echo(
            click.style("ERROR: Klien Mikrotik tidak tersedia. Tidak bisa menambahkan profil di Mikrotik.", fg="red")
        )
        return

    if db.session.execute(db.select(PackageProfile).filter_by(profile_name=name)).scalar_one_or_none():
        click.echo(
            click.style(f"ERROR: Profil Mikrotik dengan nama '{name}' sudah ada di database aplikasi.", fg="red")
        )
        return
    ...
    try:
        with get_mikrotik_connection() as mikrotik_api_conn:
            if mikrotik_api_conn:
                ...
                if mikrotik_profile_added_mt:
                    current_app.logger.info(
                        f"Profil '{name}' berhasil ditambahkan di Mikrotik. Pesan: {mikrotik_message_mt}"
                    )
                    click.echo(
                        click.style(
                            f"SUKSES: Profil '{name}' berhasil ditambahkan di Mikrotik. Pesan: {mikrotik_message_mt}",
                            fg="green",
                        )
                    )
                else:
                    current_app.logger.error(
                        f"Gagal menambahkan profil '{name}' di Mikrotik. Pesan: {mikrotik_message_mt}"
                    )
                    click.echo(
                        click.style(
                            f"ERROR: Gagal menambahkan profil '{name}' di Mikrotik. Pesan: {mikrotik_message_mt}",
                            fg="red",
                        )
                    )
            else:
        ...
        current_app.logger.error(
            f"Exception saat menambah profil Mikrotik '{name}': {str(e_mt_add_profile)}", exc_info=True
        )
        click.echo(
            click.style(f"ERROR: Exception saat menambah profil Mikrotik '{name}': {str(e_mt_add_profile)}", fg="red")
        )
    ...
    if not mikrotik_profile_added_mt:
        click.echo(
            click.style(
                "\nPERINGATAN: Profil GAGAL ditambahkan di Mikrotik. Tidak akan disimpan ke database aplikasi.",
                fg="yellow",
                bold=True,
            )
        )
    ...
    new_profile = cast(Any, PackageProfile)(profile_name=name, description=description)
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error menyimpan profil Mikrotik '{name}' ke DB setelah sukses di Mikrotik: {str(e_db_profile)}",
            exc_info=True,
        )
        click.echo(
            click.style(
                f"\nERROR DB: Gagal menyimpan profil Mikrotik ke database aplikasi meskipun sudah sukses di Mikrotik: {str(e_db_profile)}",
                fg="red",
            )
        )
        click.echo(
            click.style(
                "  Anda mungkin perlu menghapus profil ini secara manual dari Mikrotik jika tidak ingin ada duplikasi.",
                fg="yellow",
            )
        )
