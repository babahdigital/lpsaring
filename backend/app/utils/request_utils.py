...
def get_client_ip() -> Optional[str]:
    ...
    if current_app and current_app.config.get('LOG_IP_HEADER_DEBUG', False):
        ...
        for header, value in request.headers.items():
            if header.lower().startswith('x-forwarded') or \
               header.lower() in ['x-real-ip', 'remote_addr', 'host', 'user-agent', 'cf-connecting-ip']:
    ...
    log_ip_info = bool(current_app and (current_app.config.get('LOG_BINDING_DEBUG', False) or current_app.config.get('LOG_IP_HEADER_DEBUG', False)))
    if log_ip_info:
