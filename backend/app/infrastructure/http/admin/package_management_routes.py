...
def create_package(current_admin: User):
    ...
    try:
        ...
        # --- PERBAIKAN FINAL: Menggunakan nama profil dari KONFIGURASI TERPUSAT ---
        if package_data.data_quota_gb == 0:
            # Untuk paket unlimited, kita akan berikan profil unlimited murni
            target_profile_name = current_app.config['MIKROTIK_PROFILE_UNLIMITED']
        else:
            # Untuk paket kuota, profil default saat pembelian adalah 'profile-aktif'
            target_profile_name = current_app.config['MIKROTIK_PROFILE_AKTIF']
