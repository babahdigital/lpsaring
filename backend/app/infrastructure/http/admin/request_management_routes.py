...
# VERSI OPTIMAL: Menghapus helper koneksi manual dan menggunakan pool.
...
# Kode yang Benar
from app.infrastructure.http.schemas.api_schemas import RequestApprovalSchema, QuotaRequestListItemSchema, RequestApprovalAction
from app.services.notification_service import get_notification_message
from app.services import settings_service
from app.utils.formatters import format_to_local_phone

# --- [OPTIMALISASI] ---
# Impor fungsi operasi, BUKAN fungsi koneksi/diskoneksi.
...
# --- [OPTIMALISASI] ---
# Menghapus fungsi helper `_handle_mikrotik_operation` karena sudah tidak diperlukan.
# Koneksi sekarang ditangani oleh pool secara terpusat.
...
def get_all_requests(current_admin: User):
    try:
        ...
        return jsonify({"message": "Terjadi kesalahan internal saat mengambil data permintaan."}), HTTPStatus.INTERNAL_SERVER_ERROR
...
def process_quota_request(current_admin: User, request_id: uuid.UUID):
    ...
    # --- [OPTIMALISASI] ---
    # Seluruh blok try...except untuk operasi dipindahkan ke level atas
    # agar bisa menangani error dari DB dan Mikrotik sekaligus.
    try:
        if data_input.action == RequestApprovalAction.APPROVE:
            ...
            if req_to_process.request_type == RequestType.UNLIMITED:
                ...
                notification_context = {"days_added": days_to_add}
                ...
                # Pemanggilan langsung tanpa helper
                ...
                if not success:
                    raise Exception(f"Gagal sinkronisasi Mikrotik: {msg}")
            ...
            notification_context = {"request_type": req_to_process.request_type.value, "reason": data_input.rejection_reason}
            ...
            notification_context = { "reason": data_input.rejection_reason, "granted_gb": granted_gb, "granted_days": days_to_add }
        ...
        db.session.rollback()
        current_app.logger.error(f"Gagal memproses permintaan {request_id}: {e}", exc_info=True)
        return jsonify({"message": f"Gagal memproses permintaan. Error: {str(e)}"}), HTTPStatus.INTERNAL_SERVER_ERROR
    ...
    try:
        ...
        current_app.logger.error(f"Gagal mengirim notifikasi hasil proses permintaan kepada Komandan {target_user.id}: {e_notify}", exc_info=True)
    
    return jsonify({"message": "Permintaan berhasil diproses.", "new_status": req_to_process.status.name}), HTTPStatus.OK