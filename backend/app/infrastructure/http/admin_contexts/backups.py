    ...
    try:
        ...
        if result.returncode != 0:
            ...
            unsupported_dump_version = (
                extension == ".dump"
                and "unsupported version" in stderr_text.lower()
                and "file header" in stderr_text.lower()
            )
            ...
            if only_transaction_timeout_warning:
                ...
                return jsonify(
                    {"message": "Restore gagal dijalankan.", "details": stderr_text[:500]}
                ), HTTPStatus.INTERNAL_SERVER_ERROR

        return jsonify(
            {"message": "Restore database berhasil dijalankan.", "filename": safe_name, "restore_mode": restore_mode}
        ), HTTPStatus.OK
    except FileNotFoundError:
