...
def check_or_register_phone():
    ...
    try:
        ...
        if not json_data:
            ...
            return jsonify(
                {"success": False, "message": "Request body tidak boleh kosong dan harus JSON."}
            ), HTTPStatus.BAD_REQUEST
        ...
        return jsonify(
            {"success": False, "message": "Data input tidak valid.", "details": e.errors()}
        ), HTTPStatus.UNPROCESSABLE_ENTITY
    except Exception as e:
    ...
    try:
        ...
        if not user:
            ...
            return jsonify(
                PhoneCheckResponse(user_exists=False, message="Nomor telepon belum terdaftar.").model_dump()
            ), HTTPStatus.OK
        else:
            ...
            if not user.full_name and full_name:
                try:
                    ...
                    db.session.rollback()
                    current_app.logger.error(
                        f"[Check Phone] Gagal update nama untuk user {user.id}: {e_update}", exc_info=True
                    )
        ...
        return jsonify(
            {"success": False, "message": "Kesalahan internal server (check-or-register)."}
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def validate_whatsapp_number():
    ...
    try:
        ...
        if user_exists:
            ...
            return jsonify({"isValid": False, "message": "Nomor telepon ini sudah terdaftar di sistem"}), HTTPStatus.OK
    except SQLAlchemyError as e:
        current_app.logger.error(f"Database error: {str(e)}", exc_info=True)
        return jsonify({"isValid": False, "message": "Gagal memeriksa database"}), HTTPStatus.INTERNAL_SERVER_ERROR
    ...
    if not fonnte_token:
        ...
        return jsonify(
            {"isValid": False, "message": "Layanan validasi tidak terkonfigurasi"}
        ), HTTPStatus.INTERNAL_SERVER_ERROR
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return jsonify(
                {"isValid": False, "message": "Layanan validasi sementara tidak tersedia"}
        ...
        return jsonify(
            {"isValid": False, "message": "Timeout menghubungi layanan validasi"}
        ), HTTPStatus.GATEWAY_TIMEOUT
    except requests.exceptions.RequestException as e:
        current_app.logger.error(f"Fonnte connection error: {str(e)}", exc_info=True)
        return jsonify(
            {"isValid": False, "message": "Tidak dapat menghubungi layanan validasi"}
        ), HTTPStatus.SERVICE_UNAVAILABLE
    except Exception as e:
