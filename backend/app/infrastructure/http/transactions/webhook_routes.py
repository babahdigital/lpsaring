                ...
                try:
                    ...
                    try:
                        ...
                        if _user and getattr(_user, "phone_number", None):
                            ...
                            _tmpl = (
                                "user_debt_cleared_online_unblock"
                                if result.get("unblocked")
                                else "user_debt_cleared_online"
                            )
                            ...
                            _send_whatsapp_notification(
                                ...
                                _tmpl,
                                {
                                    ...
                                    "paid_manual_debt_gb": _receipt_ctx.get(
                                        "paid_manual_gb", format_mb_to_gb(_paid_manual)
                                    )
                                    if _receipt_ctx
                                    else format_mb_to_gb(_paid_manual),
                                    "paid_total_debt_gb": _receipt_ctx.get(
                                        "paid_total_gb", format_mb_to_gb(_paid_total)
                                    )
                                    if _receipt_ctx
                                    else format_mb_to_gb(_paid_total),
                                    "paid_total_debt_amount_display": _receipt_ctx.get(
                                        "paid_total_amount_display",
                                        _fmt_idr(int(getattr(transaction, "amount", 0) or 0)),
                                    )
                                    if _receipt_ctx
                                    else _fmt_idr(int(getattr(transaction, "amount", 0) or 0)),
                                    "payment_channel_label": _receipt_ctx.get(
                                        "payment_channel_label", "Pembayaran online via Midtrans"
                                    )
                                    if _receipt_ctx
                                    else "Pembayaran online via Midtrans",
                                    "payment_method_label": _receipt_ctx.get("payment_method_label", "Midtrans")
                                    if _receipt_ctx
                                    else "Midtrans",
