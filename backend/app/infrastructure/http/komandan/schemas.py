...
class QuotaRequestCreateSchema(BaseModel):
    ...
    requested_mb: int | None = Field(
        None, gt=0, description="Jumlah kuota dalam MB. Wajib jika tipe request adalah QUOTA."
    )
    requested_duration_days: int | None = Field(
        None, gt=0, description="Jumlah hari durasi. Wajib jika tipe request adalah QUOTA."
    )
