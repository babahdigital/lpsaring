...
class UserUpdateByAdminSchema(BaseModel):
    ...
    unlimited_time: Optional[bool] = Field(
        None, description="Jika true dan user unlimited, hapus quota_expiry_date (unlimited time)"
    )
    is_blocked: Optional[bool] = None
    blocked_reason: Optional[str] = None
    add_mb: Optional[int] = Field(None, ge=0)
    add_gb: Optional[float] = Field(
        None, ge=0.0, description="Kuota yang akan ditambahkan dalam Gigabyte. Akan dikonversi ke MB di backend."
    )
    add_days: Optional[int] = Field(None, ge=0)

    # Manual quota debt (MB)
    debt_package_id: Optional[uuid.UUID] = Field(
        None, description="ID paket untuk membuat debt manual berdasarkan paket"
    )
...
class UserProfileUpdateRequestSchema(BaseModel):
    ...
    def check_tamping_or_address(self) -> "UserProfileUpdateRequestSchema":
        has_address_fields = any(
            [self.blok is not None, self.kamar is not None, self.is_tamping is not None, self.tamping_type is not None]
        )
