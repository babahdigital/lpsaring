...
# PENYEMPURNAAN: Mengimplementasikan manajemen multi-perangkat berdasarkan struktur yang ada.
...
class UserKamar(enum.Enum):
    Kamar_1 = "Kamar_1"; Kamar_2 = "Kamar_2"; Kamar_3 = "Kamar_3"; Kamar_4 = "Kamar_4"; Kamar_5 = "Kamar_5"; Kamar_6 = "Kamar_6"

class TransactionStatus(enum.Enum):
    PENDING = "PENDING"; SUCCESS = "SUCCESS"; FAILED = "FAILED"; EXPIRED = "EXPIRED"; CANCELLED = "CANCELLED"; UNKNOWN = "UNKNOWN"

class NotificationType(enum.Enum):
    NEW_USER_REGISTRATION = "NEW_USER_REGISTRATION"; NEW_KOMANDAN_REQUEST = "NEW_KOMANDAN_REQUEST"; ROLE_UPGRADE_TO_ADMIN = "ROLE_UPGRADE_TO_ADMIN"; ROLE_DOWNGRADE_TO_USER = "ROLE_DOWNGRADE_TO_USER"; NEW_PACKAGE_PURCHASE = "NEW_PACKAGE_PURCHASE"

class PromoEventType(enum.Enum):
    BONUS_REGISTRATION = "BONUS_REGISTRATION"; GENERAL_ANNOUNCEMENT = "GENERAL_ANNOUNCEMENT"

class PromoEventStatus(enum.Enum):
    DRAFT = "DRAFT"; ACTIVE = "ACTIVE"; SCHEDULED = "SCHEDULED"; EXPIRED = "EXPIRED"; ARCHIVED = "ARCHIVED"

class RequestType(enum.Enum):
    QUOTA = "QUOTA"; UNLIMITED = "UNLIMITED"
...
class User(db.Model):
    ...
    blok: Mapped[Optional[UserBlok]] = mapped_column(SQLAlchemyEnum(UserBlok, name="user_blok_enum", native_enum=False), nullable=True)
    kamar: Mapped[Optional[UserKamar]] = mapped_column(SQLAlchemyEnum(UserKamar, name="user_kamar_enum", native_enum=False), nullable=True)
    ...
    is_blocked: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=expression.false(), comment="Flag untuk menandai user diblokir oleh admin.")
    blocking_reason: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="Alasan pemblokiran user oleh admin.")
    
    trusted_mac_address: Mapped[Optional[str]] = mapped_column(String(17), nullable=True, comment="MAC Address perangkat utama yang terpercaya untuk bypass.")    
    ...
    approved_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='SET NULL', name='fk_users_approved_by_id_users'), nullable=True)
    rejected_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='SET NULL', name='fk_users_rejected_by_id_users'), nullable=True)
    last_login_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    last_login_ip: Mapped[Optional[str]] = mapped_column(String(45), nullable=True, comment="Alamat IP terakhir yang digunakan user untuk login.")
    last_login_mac: Mapped[Optional[str]] = mapped_column(String(17), nullable=True, comment="Alamat MAC terakhir yang digunakan user untuk login (jika terdeteksi).")
    ...
    last_fup_notif_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True), nullable=True, comment="Waktu terakhir notifikasi status FUP aktif dikirim")
    last_habis_notif_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True), nullable=True, comment="Waktu terakhir notifikasi status KUOTA HABIS dikirim")
...
class UserDevice(db.Model):
    ...
    __table_args__ = (Index('ix_user_devices_user_id', 'user_id'), Index('ix_user_devices_mac_address', 'mac_address', unique=True),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id', ondelete="CASCADE"), nullable=False)
    # [PENYEMPURNAAN] Kolom ip_address ditambahkan agar baseline squash migration dan model konsisten.
    # Opsional untuk existing environment: jalankan migrasi add kolom jika ingin memakai kolom ini.
...
class UserLoginHistory(db.Model):
    ...
    mac_address: Mapped[Optional[str]] = mapped_column(String(17), nullable=True, comment="MAC Address yang digunakan saat login.")
...
class AddressListAudit(db.Model):
    __tablename__ = 'address_list_audit'
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[Optional[str]] = mapped_column(String(64), index=True)
    phone_comment: Mapped[Optional[str]] = mapped_column(String(32), index=True)
    old_ip: Mapped[Optional[str]] = mapped_column(String(64))
    new_ip: Mapped[Optional[str]] = mapped_column(String(64))
    old_mac: Mapped[Optional[str]] = mapped_column(String(32))
    new_mac: Mapped[Optional[str]] = mapped_column(String(32))
    action_source: Mapped[Optional[str]] = mapped_column(String(64), default='authorize-device')
