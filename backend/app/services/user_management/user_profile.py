...
def _get_active_registration_bonus() -> Optional[PromoEvent]:
    ...
    query = (
        ...
        .where(
            ...
            or_(PromoEvent.end_date.is_(None), PromoEvent.end_date >= now),
        )
...
def create_user_by_admin(admin_actor: User, data: Dict[str, Any]) -> Tuple[bool, str, Optional[User]]:
    ...
    try:
        ...
        if new_role == UserRole.SUPER_ADMIN:
            ...
            new_user.mikrotik_server_name = settings_service.get_setting(
                "MIKROTIK_DEFAULT_SERVER_KOMANDAN", "srv-komandan"
            )
            new_user.mikrotik_profile_name = (
                settings_service.get_setting("MIKROTIK_KOMANDAN_PROFILE", "komandan") or active_profile
            )
            ...
            if active_bonus and active_bonus.bonus_value_mb and active_bonus.bonus_duration_days:
                current_app.logger.info(
                    f"Menerapkan bonus registrasi '{active_bonus.name}' untuk user baru {new_user.full_name}"
                )
                new_user.total_quota_purchased_mb = active_bonus.bonus_value_mb
                new_user.quota_expiry_date = datetime.now(dt_timezone.utc) + timedelta(
...
def generate_user_admin_password(user_to_update: User, admin_actor: User) -> Tuple[bool, str]:
    ...
    default_change_password_link = current_app.config.get("APP_LINK_ADMIN_CHANGE_PASSWORD") or ""
    context = {
        "password": new_portal_password,
        "link_admin_app_change_password": settings_service.get_setting(
            "LINK_ADMIN_CHANGE_PASSWORD", default_change_password_link
        ),
    }
...
def update_user_by_admin_comprehensive(
    target_user: User, admin_actor: User, data: Dict[str, Any]
    ...
    if "role" in data and UserRole(data["role"]) != target_user.role:
        new_role = UserRole(data["role"])
        success, msg = role_service.change_user_role(
            target_user, new_role, admin_actor, data.get("blok"), data.get("kamar")
        )
        if not success:
            return False, msg, None
            ...
            try:
                ...
                _send_whatsapp_notification(
                    ...
                    "user_debt_added",
                    {
                        ...
                        "total_debt_gb": _format_total_manual_debt_gb(target_user, int(total_debt_mb)),
                        "total_manual_debt_mb": int(total_manual_debt_mb),
                        "total_manual_debt_gb": _format_total_manual_debt_gb(target_user, total_manual_debt_mb),
...
def _handle_user_activation(user: User, should_be_active: bool, admin: User) -> Tuple[bool, str]:
    ...
    if should_be_active:
        ...
        return success, msg
    current_app.logger.info(
        f"Deactivating user {user.full_name}. Setting profile to 'inactive' and limit-bytes-total to 1."
    )
    ...
    if success:
        ...
        comment = f"lpsaring|status=inactive|user={username_08}|role={user.role.value}|date={date_str}|time={time_str}"
...
def _handle_user_blocking(user: User, should_be_blocked: bool, admin: User, reason: Optional[str]) -> Tuple[bool, str]:
    ...
    if should_be_blocked:
        ...
        if success:
            ...
            username_08 = format_to_local_phone(user.phone_number) or ""
            comment = (
                f"lpsaring|status=blocked|user={username_08}|role={user.role.value}|date={date_str}|time={time_str}"
            )
            ...
            def _sync_blocked_with_fallback(api_connection, **kwargs):
                ...
                for list_name in [list_active, list_fup, list_inactive, list_expired, list_habis]:
                    if list_name and list_name != list_blocked:
                        remove_address_list_entry(
                            api_connection=api_connection, address=fallback_ip, list_name=list_name
                        )
        ...
        def _unblock_cleanup(api_connection, **kwargs):
            ...
            if ok_host and host_map and macs:
                for mac in macs:
                    ...
                    if host_ip:
                        remove_address_list_entry(
                            api_connection=api_connection, address=host_ip, list_name=list_blocked
                        )
