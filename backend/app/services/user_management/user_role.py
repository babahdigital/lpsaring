...
# --- [OPTIMALISASI] ---
# Menghapus impor _handle_mikrotik_operation
from .helpers import (
...
def change_user_role(user: User, new_role: UserRole, admin: User, blok: Optional[str] = None, kamar: Optional[str] = None) -> Tuple[bool, str]:
    ...
    if new_role == UserRole.ADMIN and old_role != UserRole.ADMIN:
        ...
        if user.blok or user.kamar:
            # Menyimpan nilai blok dan kamar sebagai string
            user.previous_blok = user.blok.value if user.blok else None
            user.previous_kamar = user.kamar.value if user.kamar else None
            user.blok, user.kamar = None, None
        
        user.role = new_role
        user.is_unlimited_user = True
        user.quota_expiry_date = None
        user.mikrotik_server_name = current_app.config.get('MIKROTIK_SERVER_ADMIN', 'srv-admin')
        user.mikrotik_profile_name = current_app.config.get('MIKROTIK_PROFILE_UNLIMITED', 'profile-unlimited')
    ...
    # Logika untuk downgrade dari Admin
        ...
        # Konversi string ke enum untuk blok dan kamar
        from app.infrastructure.db.models import UserBlok, UserKamar
        
        prev_blok_value = user.previous_blok
        prev_kamar_value = user.previous_kamar
        
        try:
            if prev_blok_value:
                user.blok = UserBlok(prev_blok_value)
            elif blok:
                user.blok = UserBlok(blok)
        except ValueError:
            user.blok = None
            
        try:
            if prev_kamar_value:
                # Format kamar value untuk enum
                kamar_value = prev_kamar_value
                if not kamar_value.startswith('Kamar_'):
                    kamar_value = f"Kamar_{kamar_value}"
                user.kamar = UserKamar(kamar_value)
            elif kamar:
                kamar_value = kamar
                if not kamar_value.startswith('Kamar_'):
                    kamar_value = f"Kamar_{kamar_value}"
        ...
        if new_role == UserRole.KOMANDAN:
            user.mikrotik_server_name = current_app.config.get('MIKROTIK_SERVER_KOMANDAN', 'srv-komandan')
            user.mikrotik_profile_name = current_app.config.get('MIKROTIK_PROFILE_UNLIMITED', 'profile-unlimited')
        else: # USER
            user.mikrotik_server_name = current_app.config.get('MIKROTIK_SERVER_USER', 'srv-user')
            user.mikrotik_profile_name = current_app.config.get('MIKROTIK_PROFILE_AKTIF', 'profile-aktif')
        
        # Menangani nilai int dari string secara aman
        initial_quota_mb = 1024  # Default value
        try:
            initial_quota_mb_str = settings_service.get_setting('USER_INITIAL_QUOTA_MB', '1024')
            if initial_quota_mb_str:
    ...
    # Logika perubahan antara User dan Komandan
        ...
        if new_role == UserRole.KOMANDAN:
            ...
            # Menangani nilai int dari string secara aman
            initial_quota_mb = 5120  # Default value
            try:
            ...
            # Menangani nilai int dari string secara aman
            initial_duration_days = 30  # Default value
            try:
            ...
            try:
                user.blok = UserBlok(blok) if blok else None
            except ValueError:
                user.blok = None
                
            try:
                kamar_value = kamar
                if kamar and not kamar.startswith('Kamar_'):
                    kamar_value = f"Kamar_{kamar}"
                user.kamar = UserKamar(kamar_value) if kamar_value else None
            except ValueError:
                user.kamar = None
    ...
    if mikrotik_username:
        ...
        # --- [OPTIMALISASI] ---
        # Mengganti pemanggilan _handle_mikrotik_operation dengan pemanggilan langsung.
        # Pastikan mikrotik_profile_name selalu string, tidak None
        profile_name = current_app.config.get('MIKROTIK_PROFILE_AKTIF', 'profile-aktif')  # Default profile sebagai fallback terakhir
        
        if user.mikrotik_profile_name:
            profile_name = str(user.mikrotik_profile_name)
        else:
            # Sesuaikan profile berdasarkan role user
            if user.role == UserRole.KOMANDAN:
                profile_name = current_app.config.get('MIKROTIK_PROFILE_UNLIMITED', 'profile-unlimited')
            elif user.role == UserRole.ADMIN or user.role == UserRole.SUPER_ADMIN:
                profile_name = current_app.config.get('MIKROTIK_PROFILE_ADMIN', 'profile-admin')
            else:
                profile_name = current_app.config.get('MIKROTIK_PROFILE_AKTIF', 'profile-aktif')
            
        mikrotik_op_success, mikrotik_op_message = activate_or_update_hotspot_user(
