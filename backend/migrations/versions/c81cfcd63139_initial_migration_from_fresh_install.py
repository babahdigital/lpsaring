...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=10),
            nullable=True,
            comment="Menyimpan data blok terakhir sebelum diupgrade ke admin",
        ),
        sa.Column(
            "previous_kamar",
            sa.String(length=20),
            nullable=True,
            comment="Menyimpan data kamar terakhir sebelum diupgrade ke admin",
        ),
        sa.Column("is_active", sa.Boolean(), server_default=sa.text("false"), nullable=False),
        sa.Column(
            "role",
            sa.Enum("USER", "KOMANDAN", "ADMIN", "SUPER_ADMIN", name="user_role_enum", native_enum=False),
            nullable=False,
        ),
        sa.Column(
            "approval_status",
            sa.Enum("PENDING_APPROVAL", "APPROVED", "REJECTED", name="approval_status_enum", native_enum=False),
            nullable=False,
        ),
        sa.Column(
            "mikrotik_user_exists",
            sa.Boolean(),
        ...
        sa.Column(
            "mikrotik_server_name",
            sa.String(length=100),
            nullable=True,
            comment="Nama server hotspot spesifik di Mikrotik.",
        ),
        sa.Column(
            "mikrotik_profile_name",
            sa.String(length=100),
            nullable=True,
            comment="Nama profil hotspot spesifik di Mikrotik.",
        ),
        ...
        sa.Column(
            "last_low_quota_notif_at",
            sa.DateTime(timezone=True),
            nullable=True,
            comment="Waktu terakhir notifikasi kuota rendah dikirim",
        ),
        sa.Column(
            "last_expiry_notif_at",
            sa.DateTime(timezone=True),
            nullable=True,
            comment="Waktu terakhir notifikasi akan kedaluwarsa dikirim",
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"], ["users.id"], name="fk_daily_usage_logs_user_id_users", ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column(
            "event_type",
            sa.Enum("BONUS_REGISTRATION", "GENERAL_ANNOUNCEMENT", name="promo_event_type_enum", native_enum=False),
            nullable=False,
        ),
        sa.Column(
            "status",
        ...
        sa.Column(
            ...
            sa.BigInteger(),
            nullable=True,
            comment="Nilai bonus dalam MB untuk event tipe BONUS_REGISTRATION",
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "request_type", sa.Enum("QUOTA", "UNLIMITED", name="request_type_enum", native_enum=False), nullable=False
        ),
        sa.Column(
            "request_details",
            sa.Text(),
            nullable=True,
            comment="Detail request dlm JSON, misal: {'mb_to_add': 10240, 'days_to_add': 30}",
        ),
        sa.Column(
            "granted_details",
            sa.Text(),
            nullable=True,
            comment="Detail persetujuan parsial dlm JSON, misal: {'granted_mb': 5120, 'granted_duration_days': 15}",
        ),
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.ForeignKeyConstraint(
            ["package_id"], ["packages.id"], name="fk_transactions_package_id_packages", ondelete="RESTRICT"
        ),
