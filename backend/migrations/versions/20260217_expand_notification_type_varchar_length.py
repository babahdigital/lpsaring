...
def upgrade():
    ...
    op.execute("ALTER TABLE notification_recipients ALTER COLUMN notification_type TYPE VARCHAR(64);")


def downgrade():
    # Best-effort rollback. This can fail if rows already contain values longer than 22 chars.
    op.execute("ALTER TABLE notification_recipients ALTER COLUMN notification_type TYPE VARCHAR(22);")
