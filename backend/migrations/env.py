    ...
    try:
        with connectable.connect().execution_options(isolation_level="AUTOCOMMIT") as preflight_conn:
            preflight_conn.execute(
                text("CREATE TABLE IF NOT EXISTS alembic_version (version_num VARCHAR(64) NOT NULL PRIMARY KEY)")
            )
            preflight_conn.execute(text("ALTER TABLE alembic_version ALTER COLUMN version_num TYPE VARCHAR(64)"))
    except Exception:
