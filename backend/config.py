...
class Config:
    ...
    if not SQLALCHEMY_DATABASE_URI:
        ...
        if db_user and db_password and db_host and db_name:
            ...
            if is_testing:
                ...
                warnings.warn("PERINGATAN: DATABASE_URL tidak ditemukan. Menggunakan fallback sqlite in-memory untuk testing.")
            else:
    ...
    AUTH_COOKIE_MAX_AGE_SECONDS = get_env_int(
    ...
    QUOTA_FUP_PERCENT = get_env_int('QUOTA_FUP_PERCENT', 20)
    QUOTA_NOTIFY_PERCENTAGES = get_env_list('QUOTA_NOTIFY_PERCENTAGES', "[20, 10, 5]")
    ...
    APP_LINK_ADMIN = os.environ.get('APP_LINK_ADMIN') or (f"{APP_LINK_USER.rstrip('/')}/admin" if APP_LINK_USER else None)
    APP_LINK_MIKROTIK = os.environ.get('APP_LINK_MIKROTIK')
    APP_LINK_ADMIN_CHANGE_PASSWORD = os.environ.get('APP_LINK_ADMIN_CHANGE_PASSWORD') or (f"{APP_LINK_ADMIN.rstrip('/')}/account-settings" if APP_LINK_ADMIN else None)
    ...
    def validate_production_config(cls):
        if cls.FLASK_ENV == 'production':
            ...
            if cls.ENABLE_WHATSAPP_NOTIFICATIONS and not cls.WHATSAPP_VALIDATE_URL:
                warnings.warn("PERINGATAN PRODUKSI: WHATSAPP_VALIDATE_URL tidak disetel. Validasi nomor WhatsApp akan gagal.")
