...
class MockAppConfig:
    ...
    def __init__(self):
        ...
        api_key_display = '********' + api_key[-4:] if api_key and len(api_key) > 4 else ('Set' if api_key else 'Not Set')
...
is_invalid_format = not current_test_number.startswith('+628') or len(current_test_number) < 11 # Cek panjang minimal dasar

if is_still_placeholder or is_invalid_format:
    print("\n!!! PERHATIAN !!!")
    # Tampilkan nomor yang *sebenarnya* ada di variabel saat ini
    print(f"Nomor tujuan tes ('{current_test_number}') masih berupa placeholder ('{placeholder_number}') atau formatnya tidak valid.")
    print("Mohon edit file tests/test_whatsapp_send.py dan ganti placeholder dengan nomor WhatsApp Anda yang valid (diawali +62).")
...
if success:
    ...
    print("   Periksa log error di atas (jika ada dari fungsi send_whatsapp_message), pastikan API Key Fonnte di .env sudah benar,")
