...
- **Dialog admin mobile kini lebih ringan disentuh dan lebih mudah dipindai:** area `Koreksi Kuota Langsung` diringkas agar informasi satuan tidak berulang, ringkasan nilai saat ini dipisah ke kartu statistik yang lebih mudah dibaca, aksi `Tunggakan Kuota` dinaikkan menjadi kartu operasional dengan tombol detail/popup yang jelas, dan dialog admin yang memakai `AppPerfectScrollbar` kini otomatis beralih ke native scroll saat mobile untuk menghindari lag sentuhan dari listener `perfect-scrollbar`.
