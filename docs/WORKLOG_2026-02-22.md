...
Healthcheck (jika port 80 tidak publish ke host, **konteks legacy saat nginx masih di app compose**):
```bash
docker compose --env-file .env.prod -f docker-compose.prod.yml exec -T nginx wget -qO- http://127.0.0.1/api/ping
```

Catatan arsitektur aktif (DO split-stack): healthcheck dilakukan via `docker exec global-nginx-proxy ...`.
