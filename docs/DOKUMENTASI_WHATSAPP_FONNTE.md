...
### 2.6 Pembaruan Implementasi (2026-02-07)
1) **Lokasi DB WhatsApp**
  - Log & recipients disimpan di **App DB**: `db_data/babahdigital_app.db`.
  - Migrasi otomatis dari main DB jika tabel kosong.
2) **Log per target**
  - Multi-target dipisah menjadi log per nomor/grup.
  - Kolom tambahan: `request_id`, `message_id`, `status_detail`, `updated_at`.
3) **Status tanpa webhook**
  - Respons `process: pending/queue` ditampilkan sebagai **success** (permintaan diterima).
  - **Invalid** tetap ditandai jika nomor tidak terdaftar di WhatsApp.
4) **Webhook status (opsional)**
  - Endpoint: `/system/whatsapp/webhook.php`.
  - Token opsional via env: `security.whatsapp_webhook_token`.
  - Jika aktif, status bisa berubah `sent/failed` setelah update Fonnte.
  - **Wajib URL publik HTTPS** (mis. Cloudflare Tunnel) agar webhook bisa diakses dari luar.
5) **Validasi nomor saat tambah admin/operator**
  - Nomor wajib dan diverifikasi via API Fonnte sebelum disimpan.
  - Format diterima: **08** atau **62**, panjang **10–14 digit**.

---

## 3) Integrasi Akun Admin/Operator
1) **Template akun**
  - `account_register`: kirim kredensial saat pendaftaran akun admin/operator.
  - `account_reset`: kirim password baru saat reset.
  - Format waktu: **dd-mm-yyyy HH:ii**.

2) **Created by**
  - Menggunakan **nama lengkap** bila tersedia.
  - Fallback ke **username** (termasuk akun ENV tanpa nama).

3) **Recipient switches**
  - Toggle kategori (Retur/Report/L/S/Todo) benar‑benar memfilter target.
  - Jika kategori tidak memiliki penerima aktif, **pesan tidak dikirim** (tanpa fallback ke default).

4) **Pembersihan otomatis**
  - Saat admin/operator dihapus, entry `whatsapp_recipients` dihapus berdasarkan nomor/label.

5) **Config source**
  - Config WhatsApp dibaca dari DB (`whatsapp_config`).
  - Jika DB kosong, fallback dari env (endpoint/token/target/country_code/timezone).
