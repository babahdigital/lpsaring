...
Catatan (update): bagian ini awalnya membahas threshold berbasis persen. Sistem saat ini sudah pindah ke threshold berbasis MB (`QUOTA_FUP_THRESHOLD_MB` dan `QUOTA_NOTIFY_REMAINING_MB`). Poin 20%/10%/5% di bawah bersifat historis.
...
### Solusi
1) Backend: gunakan low-quota threshold berbasis **MB** agar tidak bergantung persentase.
2) Produksi: set env/setting:
...
### Verifikasi
- Cek setting DB/env:
  - `QUOTA_NOTIFY_REMAINING_MB` sesuai kebutuhan (contoh `[500]`).
- Cek source running container:
  - `_send_quota_notifications` memakai `QUOTA_NOTIFY_REMAINING_MB`.
...
- DB/env `QUOTA_NOTIFY_REMAINING_MB=[500]` tersimpan.

---

## 5) Quota-debt cap: hard block saat hutang melewati limit

> Update 2026-03-15: bagian ini bersifat historis. Policy aktif sekarang adalah auto debt hard-block dengan ip-binding tetap `regular`, WA debt-limit ke user hanya pada transisi block baru, dan fan-out admin berada di jalur task/admin terpisah. Rujuk `docs/RULES_QUOTA_DEBT_ACCESS_FLOW.md` dan runbook aktif untuk perilaku terkini.
