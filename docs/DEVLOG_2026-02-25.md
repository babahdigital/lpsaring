...
## 12) Update CI Fokus Frontend Test (Auth + Payment)

Perubahan lanjutan pada pipeline CI frontend:

- Step focused unit test diperluas dari auth-only menjadi **auth + payment**.
- Daftar test focused sekarang mencakup:
  - `tests/auth-access.test.ts`
  - `tests/auth-guards.test.ts`
  - `tests/payment-composables.test.ts`
  - `tests/payment-status-polling.test.ts`
- Ditambahkan manifest focused tests ke `GITHUB_STEP_SUMMARY` agar lebih mudah diaudit saat troubleshooting.
- Daftar file test focused didefinisikan sekali (`FOCUSED_FRONTEND_TESTS`) lalu dipakai ulang pada step eksekusi test.

Tujuan:
- Mengunci regresi alur payment composables di pipeline yang cepat (focused tests), tanpa menunggu full suite.

## 13) Modularisasi Admin Routes + Hardening Test Coverage

Perubahan lanjutan yang sudah selesai:

- `admin_routes.py` dirampingkan menjadi delegasi tipis ke bounded context:
  - `backend/app/infrastructure/http/admin_contexts/backups.py`
  - `backend/app/infrastructure/http/admin_contexts/notifications.py`
  - `backend/app/infrastructure/http/admin_contexts/transactions.py`
  - `backend/app/infrastructure/http/admin_contexts/billing.py`
  - `backend/app/infrastructure/http/admin_contexts/reports.py`
- Refactor menjaga kompatibilitas route existing (tanpa ubah endpoint publik).
- Event transaksi distandarkan memakai payload schema `transaction_event.v1` untuk konsistensi observability.
- Frontend payment status diperkuat dengan unit test:
  - `frontend/tests/payment-composables.test.ts`
  - `frontend/tests/payment-status-polling.test.ts`

Validasi utama:
- Targeted backend tests (admin + transaction lifecycle + openapi smoke) lulus.
- Focused frontend tests (auth + payment) lulus.
- Lint/diagnostics file yang disentuh dalam refactor bersih.
