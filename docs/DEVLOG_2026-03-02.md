...
## Addendum Insiden Produksi (2026-03-02, setelah deploy)

### Ringkasan Insiden
- Terjadi operasi deploy bersih yang menyebabkan risiko kehilangan backup aktif dan database sempat kosong.
- Data `users` berhasil dipulihkan dari backup SQL lokal (`backend/backups/cek_data_user.sql`) ke produksi.
- Setelah fase operasional lanjutan, terjadi insiden lanjutan: user hotspot pada MikroTik dilaporkan hilang.

### Dampak
- Akses internet user terdampak karena entri `/ip/hotspot/user` berkurang/hilang.
- DB aplikasi tetap menjadi sumber kebenaran utama (data user ada), namun state router tidak sinkron.

### Tindakan yang Sudah Dilakukan
- Penguatan command backend agar bulk quota dapat dibatasi scope server (`srv-user`) dan tidak memaksa profile expired jika tidak diminta.
- Penyusunan runbook recovery darurat untuk rekonstruksi user MikroTik dari DB dengan scope ketat `srv-user`.
- Eskalasi operasional: perintah recovery dijalankan hanya setelah verifikasi koneksi SSH, path deploy, dan compose file produksi.

### Keputusan Operasional Final untuk Insiden Ini
- Prioritas pemulihan: **restore user MikroTik dari DB** terlebih dahulu.
- Scope eksekusi: **hanya `mikrotik_server_name='srv-user'`**.
- Larangan saat fase recovery: tidak melakukan operasi tambahan yang mengubah policy/profile lintas server.

### Preventive Guardrail Tambahan
- Semua operasi destruktif wajib didahului checkpoint verifikasi backup yang benar-benar dapat diakses.
- Semua command bulk yang menyentuh MikroTik wajib memiliki filter scope eksplisit (minimal server).
- Jalankan dry-run/diagnostik hitung target sebelum apply.

---

## Addendum Dashboard Admin (2026-03-02 malam)

### Latar Belakang
- User melaporkan KPI dashboard `Pendapatan Bulan Ini` dan `Kuota Terjual` terlihat kosong, padahal sebelumnya sempat terisi.
- User juga meminta verifikasi komponen dashboard admin lain dan deploy ulang dengan mode prune ke host produksi.

### Investigasi Produksi
- Endpoint dashboard dan pendukung (`/api/admin/dashboard/stats`, `/api/admin/metrics`, `/api/admin/metrics/access-parity`, `/api/admin/backups`) terkonfirmasi merespons `HTTP 200` dari log backend.
- Query transaksi produksi menunjukkan status `SUCCESS` ada dan data paket valid (`Paket Hemat`, `data_quota_gb=10.00`), sehingga sumber data KPI tersedia.
- Agregat month-to-date dari DB terverifikasi: `success_month=2`, `revenue_month=200000`, `quota_gb_month=20.00`.

### Root Cause Teknis
- Bukan outage endpoint dashboard; endpoint berjalan normal.
- Ditemukan mismatch mapping pada kartu ringkasan atas di halaman admin users (nilai indikator tidak sesuai label).
- Untuk panel `Preview Cleanup Nonaktif`, UX belum menyembunyikan section saat kandidat kosong, sehingga memunculkan noise visual.

### Perbaikan yang Diterapkan
- Backend `admin_routes.py`:
  - Menambah metrik `menungguPersetujuan` dari count user `approval_status=PENDING` pada payload `/dashboard/stats`.
- Frontend `pages/admin/dashboard.vue`:
  - Memperbaiki mapping kartu ringkasan agar sesuai label metrik (`Akan Kadaluwarsa` dan `Menunggu Persetujuan`).
- Frontend `pages/admin/users.vue`:
  - Menyembunyikan seluruh card `Preview Cleanup Nonaktif` bila tidak ada kandidat pada daftar top deactivate/delete.

### Validasi Pasca Perbaikan
- Deploy ulang dijalankan dengan `deploy_pi.sh --prune` ke `/home/abdullah/sobigidul`.
- Health stack pasca deploy: frontend, db, redis berstatus `healthy`; `/api/ping` via nginx mengembalikan `pong`.
- Commit perubahan aplikasi dan UX sudah dipush ke branch `main`.
