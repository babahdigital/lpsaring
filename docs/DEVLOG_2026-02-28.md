...
Masalah awal:
- validasi terlalu menekankan `/ip/hotspot/active`, berisiko memaksa relogin padahal device sah via ip-binding.

Perubahan:
- `hotspot_session_active` diturunkan dari kepemilikan ip-binding user (bukan active-only).
...
Perubahan:
- redirect saat `hotspot_login_required && hotspot_session_active !== true` (termasuk state unknown/null).
...
- Penghapusan fallback auto-login by active session IP (`/ip/hotspot/active`) agar ownership tetap berbasis device authorized.
