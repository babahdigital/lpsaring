...
export function getClientRealIP(): string | null {
    ...
    if (captiveIP) {
        try { localStorage.setItem('captive_ip', captiveIP) } catch { /* noop */ }
        return captiveIP
    }
...
export function getBestClientInfo() {
    ...
    if (urlIp) { try { localStorage.setItem('captive_ip', urlIp) } catch { /* noop */ } }
    if (urlMac) { try { localStorage.setItem('captive_mac', urlMac) } catch { /* noop */ } }
