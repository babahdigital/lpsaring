...
describe('hotspotPostLoginBridge', () => {
  it('starts a bridge when post-login flow still has no hotspot identity and a trusted hint exists', () => {
    expect(shouldAttemptPostLoginHotspotBridge(
      { clientIp: '', clientMac: '' },
      'http://login.home.arpa/login',
      'http://login.home.arpa/login',
    )).toBe(true)
  })
  ...
  it('skips the bridge when an explicit hotspot identity already exists', () => {
    expect(shouldAttemptPostLoginHotspotBridge(
      { clientIp: '172.16.2.10', clientMac: '' },
      'http://login.home.arpa/login',
      'http://login.home.arpa/login',
    )).toBe(false)
    expect(shouldAttemptPostLoginHotspotBridge(
      { clientIp: '', clientMac: 'AA:BB:CC:DD:EE:FF' },
      'http://login.home.arpa/login',
      'http://login.home.arpa/login',
    )).toBe(false)
  })
  ...
  it('skips the bridge when no target URL is available', () => {
    expect(shouldAttemptPostLoginHotspotBridge({ clientIp: '', clientMac: '' }, '', 'http://login.home.arpa/login')).toBe(false)
  })

  it('skips the bridge when no trusted hotspot hint exists', () => {
    expect(shouldAttemptPostLoginHotspotBridge({ clientIp: '', clientMac: '' }, 'http://login.home.arpa/login', '')).toBe(false)
  })
