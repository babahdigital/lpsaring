...
const resetLoginPending = ref(false)
...
const showResetLoginButton = computed(() => isLoggedIn.value && currentUser.value != null && !isAdmin.value)
const isResetLoginLoading = computed(() => resetLoginPending.value || authStore.resetLoginInProgress)

async function handleResetLogin() {
  if (isResetLoginLoading.value)
    return
  resetLoginPending.value = true
  try {
    await authStore.resetLogin()
  }
  finally {
    resetLoginPending.value = false
  }
}
...
<template>
  ...
  >
    <VBtn
      v-if="showResetLoginButton"
      color="warning"
      variant="tonal"
      size="small"
      :loading="isResetLoginLoading"
      :disabled="isResetLoginLoading"
      class="me-2"
