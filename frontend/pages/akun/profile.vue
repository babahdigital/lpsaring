...
const LoginHistoryCard = defineAsyncComponent(() => import('~/components/akun/LoginHistoryCard.vue'))
const UserSpendingChart = defineAsyncComponent(() => import('~/components/charts/UserSpendingChart.vue'))
...
const displayKamar = computed(() => {
  const kamar = currentUser.value?.kamar
  if (kamar && kamar.startsWith('Kamar_')) {
    // Jika formatnya 'Kamar_X', ambil bagian setelah 'Kamar_'
    return kamar.substring(6)
  }
  // Jika formatnya lain atau kosong, tampilkan apa adanya atau 'N/A'
  return kamar || 'N/A'
})
// --- [AKHIR PENAMBAHAN] ---
...
async function loadInitialData() {
  ...
  try {
    // Check for admin flag in localStorage - if present but no role, we need to fix this
    const isAdminInLocalStorage = localStorage.getItem('is_admin_user') === 'true'
    const userHasNoRole = !authStore.currentUser?.role
    const userRoleInconsistent = isAdminInLocalStorage && !['ADMIN', 'SUPER_ADMIN'].includes(authStore.currentUser?.role || '')

    console.log('[PROFILE] User data check - localStorage admin flag:', isAdminInLocalStorage, 'Current role:', authStore.currentUser?.role)

    // Force refresh if user data is missing or inconsistent
    if (authStore.currentUser === null || authStore.currentUser === undefined || userHasNoRole || userRoleInconsistent) {
      console.log('[PROFILE] User data missing or inconsistent, fetching fresh data')
      const userData = await authStore.fetchUser()

      // If role is still missing but admin flag is in localStorage, verify with backend
      if ((userData?.role === undefined || !userData?.role) && isAdminInLocalStorage) {
        console.log('[PROFILE] Still no role after fetch but admin flag exists, verifying with backend')
        try {
          const { $api } = useNuxtApp()
          const roleVerification = await $api('/auth/verify-role')
          if (roleVerification?.isAdmin === true && authStore.currentUser) {
            console.log('[PROFILE] Backend confirmed admin status as:', roleVerification.role)
            authStore.setUser({
              ...authStore.currentUser,
              role: roleVerification.role || 'ADMIN',
            })
          }
        }
        catch (error) {
          console.error('[PROFILE] Role verification failed:', error)
        }
      }

      if (!userData && !authStore.currentUser)
...
// Fungsi reset hotspot password telah dihapus karena tidak lagi diperlukan
async function fetchSpendingSummary() {
  spendingChartLoading.value = true
  spendingAlert.value = null
  try {
    const response = await $api<{ categories: string[], series: any[], total_this_week: number }>('/dashboard/weekly-spending')
              ...
              <VCardText>
                ...
                <VForm ref="profileForm" @submit.prevent="saveProfile">
                  <VRow>
                    ...
                    <VCol v-if="isUser" cols="12" md="6">
                      <AppTextField :model-value="displayKamar || 'N/A'" label="Kamar" readonly disabled prepend-inner-icon="tabler-door" />
                    </VCol>
                  </VRow>
            ...
            <VCard>
              ...
              <VCardText>
                ...
                <div class="d-flex flex-wrap ga-4">
                  ...
                  <!-- Tombol reset password hotspot telah dihapus -->
                </div>
              </VCardText>
            </VCard>
          </VCol>
            ...
            <VCard>
              ...
              <VCardText>
                ...
                <div :style="{ height: display.smAndDown ? '200px' : '250px' }">
                  <UserSpendingChart v-if="spendingChartData[0] && spendingChartData[0].data.length > 0" :series-data="spendingChartData" :categories="spendingChartCategories" />
                </div>
              </VCardText>
            </VCard>
          </VCol>
