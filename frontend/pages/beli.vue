...
// [PERUBAHAN 1] Impor composable useMidtrans
...
// [PERUBAHAN 2] Inisialisasi composable
...
function handlePackageSelection(pkg: Package) {
  if (pkg?.id == null || pkg.is_active !== true || isInitiatingPayment.value != null)
    return
  if (!isUserApprovedAndActive.value) {
    console.warn('Percobaan pembelian oleh pengguna yang belum disetujui/aktif dicegah.')
    return
  }
...
async function initiatePayment(packageId: string) {
  ...
  try {
    ...
    // [PERUBAHAN 3] Ganti logika window.snap dengan memanggil fungsi dari composable
    if (responseData?.snap_token) {
      // Panggil fungsi `pay` dari `useMidtrans`, yang akan menangani
      // pemuatan skrip dan menampilkan jendela pembayaran.
                    ...
                    >
                      ...
                      <VCardText class="flex-grow-1 py-2 text-left">
                        <VList lines="one" density="compact" bg-color="transparent" class="py-0">
                          <VListItem>
                            <template #prepend>
                              <div class="me-2" style="width: 20px; height: 20px;">
                                <!-- ICON UPDATE: database -->
                          ...
                          <VListItem>
                            <template #prepend>
                              <div class="me-2" style="width: 20px; height: 20px;">
                                <!-- ICON UPDATE: bolt -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="text-success" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M13 3l-7 11h9l-7 7" /></svg>
                              </div>
                            </template>
                            <VListItemTitle class="text-body-2">
