...
const headers = computed(() => {
  const baseHeaders = [
    ...
    { title: 'Jumlah', key: 'amount', sortable: true, align: 'end' as const },
...
const { data: apiResponse, pending: loading, error: fetchError, refresh: _loadItems } = useApiFetch<PaginatedResponse<Transaction>>(
  computed(() => `/users/me/transactions?${queryParams.value.toString()}`),
  {
    method: 'GET',
    watch: [queryParams],
    server: false,
...
watch(apiResponse, (newData) => {
  // PERBAIKAN: Pengecekan boolean eksplisit dan type assertion
  const typedData = newData as PaginatedResponse<Transaction>
  if (typedData?.success === true && Array.isArray(typedData.data)) {
    transactions.value = typedData.data

    // PERBAIKAN: Pengecekan null eksplisit
    if (typedData.pagination != null) {
      totalItems.value = typedData.pagination.total_items
      currentPage.value = typedData.pagination.page
      itemsPerPage.value = typedData.pagination.per_page
    }
  }
  // PERBAIKAN: Pengecekan null dan boolean eksplisit
  else if (typedData != null && typedData.success === false) {
    transactions.value = []
    totalItems.value = 0
  }
}, { immediate: true })
...
async function downloadInvoice(midtransOrderId: string) {
  ...
  try {
    const response = await fetch(`/api${invoiceUrl}`, {
      method: 'GET',
      headers: {
