...
watch(error, (newError) => {
  if (newError) {
    ...
    showSnackbar({ type: 'error', title: 'Gagal Memuat Log', text: errorData?.message ?? 'Terjadi kesalahan server.' })
  }
})
...
const headers = computed(() => {
  const base = [
    ...
    { title: 'Aksi', key: 'action_type', align: 'center' as const, width: '220px' },
