...
const fetchLoginHistory = async () => {
  ...
  try {
    ...
    if (Array.isArray(response)) {
        ...
        throw new Error('Format respons data riwayat tidak valid.');
    }
  } catch (error: any) {
  ...
  <div>
    ...
    <VList v-else-if="loginHistory.length > 0" lines="two" density="compact" class="card-list pb-8 ma-6">
      <VListItem v-for="(item, index) in loginHistory" :key="index">
        ...
        <VListItemTitle class="font-weight-semibold text-subtitle-1">{{ item.device }} ({{ item.os }})</VListItemTitle>
        <VListItemSubtitle class="text-caption">IP: {{ item.ip_address }}</VListItemSubtitle>
        <template #append>
