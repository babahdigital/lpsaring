...
interface User {
  ...
  is_active: boolean; blok: string | null; kamar: string | null; is_unlimited_user: boolean;
  mikrotik_server_name: string | null; mikrotik_profile_name: string | null;
...
function getInitialFormData(): Partial<User & { add_gb: number, add_days: number }> {
  return {
    full_name: '', phone_number: '', role: 'USER', is_active: true, blok: null, kamar: null, is_unlimited_user: false,
    mikrotik_server_name: null, mikrotik_profile_name: null, total_quota_purchased_mb: 0, total_quota_used_mb: 0,
    quota_expiry_date: null, add_gb: 0, add_days: 0,
  }
}
...
const canAdminInject = computed(() => {
    ...
    if (isRestrictedAdmin.value && formData.role === 'KOMANDAN') return true
    return false
})

const isSaveDisabled = computed(() => {
  if (!isRestrictedAdmin.value) return false;
...
watch(() => props.user, (newUser) => {
    if (newUser) {
        ...
        if (authStore.isAdmin && !authStore.isSuperAdmin) {
            if (formData.mikrotik_server_name === 'srv-admin') adminAccessType.value = 'admin'
            else if (formData.mikrotik_server_name === 'srv-komandan') adminAccessType.value = 'komandan'
...
const setDefaultMikrotikConfig = (role: User['role'] | undefined) => {
    switch (role) {
        case 'USER':
...
watch(() => formData.is_unlimited_user, (isUnlimited, wasUnlimited) => {
    if (isUnlimited === wasUnlimited) return;
...
const checkAndApplyMikrotikStatus = async () => {
    ...
    try {
        ...
        if (response.exists_on_mikrotik && response.details) {
            ...
            const usage_bytes = parseInt(response.details['bytes-in'] || '0') + parseInt(response.details['bytes-out'] || '0');
            liveData.value = {
                isAvailable: true,
    ...
    <VCard>
      <VForm ref="formRef" @submit.prevent="onSave">
               ...
               <VRow>
                ...
                <template v-if="canAdminInject && formData.is_active">
                  ...
                  <VCol cols="12" class="d-flex justify-space-between align-center">
                    <div class="text-overline">Inject Kuota & Masa Aktif</div>
                    <VBtn
                        v-if="authStore.isSuperAdmin"
                        size="small" variant="tonal" color="info"
                        @click="checkAndApplyMikrotikStatus" :loading="isCheckingMikrotik"
                  ...
                  <VCol v-if="liveData" cols="12">
                    <VSheet rounded="lg" border class="pa-3 mb-4">
                      <VRow dense>
                        <VCol cols="12" sm="6"><div class="text-caption text-disabled">Sisa Kuota (DB)</div><div class="font-weight-medium">{{ liveData.db_sisa_kuota_mb.toFixed(2) }} MB</div></VCol>
                        <VCol cols="12" sm="6"><div class="text-caption text-disabled">Pemakaian (MikroTik)</div><div class="font-weight-medium">{{ liveData.mt_usage_mb.toFixed(2) }} MB</div></VCol>
                        <VCol cols="12"><div class="text-caption text-disabled">Masa Aktif (DB)</div><div class="font-weight-medium">{{ liveData.db_expiry_date ? new Date(liveData.db_expiry_date).toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' }) : 'Tidak ada' }}</div></VCol>
                      </VRow>
                    </VSheet>
                  </VCol>
                  ...
                  <VCol v-if="authStore.isSuperAdmin" cols="12">
                    <div class="text-overline mb-3">Pengaturan Mikrotik</div>
        ...
        <VCardActions class="pa-4 d-flex">
          <div v-if="props.user">
             <VBtn v-if="!isTargetAdminOrSuper && formData.is_active" color="warning" variant="text" prepend-icon="tabler-key" @click="$emit('reset-hotspot', props.user.id)">Reset Pass Mikrotik</VBtn>
              <VMenu v-else-if="formData.is_active" location="top">
                <template #activator="{ props: menuProps }">
                  <VBtn color="warning" variant="text" prepend-icon="tabler-key" v-bind="menuProps">Reset Password</VBtn>
                </template>
                <VList>
                  <VListItem @click="$emit('reset-hotspot', props.user.id)" title="Password Mikrotik" prepend-icon="tabler-wifi" />
                  <VListItem @click="$emit('generate-admin-pass', props.user.id)" title="Password Portal" prepend-icon="tabler-shield-lock" />
                </VList>
              </VMenu>
          </div>
          <VSpacer />
          <VBtn variant="tonal" color="secondary" @click="onClose">Batal</VBtn>
          <VBtn type="submit" color="primary" :loading="props.loading" :disabled="isSaveDisabled" prepend-icon="tabler-device-floppy">Simpan Perubahan</VBtn>
        </VCardActions>
      </VForm>
    </VCard>
  </VDialog>
