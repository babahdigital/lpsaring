...
interface QuotaInfo {
  totalQuotaMB: number; usedQuotaMB: number; remainingQuotaMB: number;
  percentageUsed: number; isUnlimited: boolean; expiryDate: string | null;
  remainingDays: number | null; status: 'ACTIVE' | 'EXPIRED' | 'NOT_SET' | 'UNLIMITED';
  statusColor: string;
}
...
const formatDataSize = (sizeInMB: number) => {
  if(isNaN(sizeInMB)) return '0 MB';
  const options = { minimumFractionDigits: 2, maximumFractionDigits: 2 };
  if (sizeInMB < 1) return `${(sizeInMB * 1024).toLocaleString('id-ID', options)} KB`
  else if (sizeInMB < 1024) return `${sizeInMB.toLocaleString('id-ID', options)} MB`
...
const quotaDetails = computed((): QuotaInfo | null => {
  ...
  if (props.user.role === 'ADMIN' || props.user.role === 'SUPER_ADMIN') return null
  ...
  let status: QuotaInfo['status'] = 'NOT_SET', statusColor = 'grey', remainingDays: number | null = null
  ...
  if (user.is_unlimited_user) {
    ...
    if (user.quota_expiry_date) {
        ...
        if (isValid(expiry)) remainingDays = Math.max(0, differenceInDays(expiry, new Date()));
    }
  ...
  return {
    totalQuotaMB: totalMB, usedQuotaMB: usedMB, remainingQuotaMB: remainingMB, percentageUsed: percentage,
    ...
    remainingDays, status, statusColor,
  }
})
  ...
  <VDialog :model-value="props.modelValue" max-width="600" persistent scrollable @update:model-value="onClose">
    <VCard v-if="props.user">
      ...
      <VCardText class="pa-5" style="max-height: 70vh; overflow-y: auto;">
        <div class="text-overline mb-2">Informasi Dasar</div>
        <VList lines="two" density="compact">
          ...
          <VListItem prepend-icon="tabler-shield-check" title="Peran">
            <template #subtitle>
              <div class="d-flex align-center">
                <VChip :color="roleMap[props.user.role]?.color" size="small" label>{{ roleMap[props.user.role]?.text }}</VChip>
          ...
          <VListItem prepend-icon="tabler-checkup-list" title="Status Akun">
            <template #subtitle>
              <VChip :color="statusMap[props.user.approval_status]?.color" size="small" label>{{ statusMap[props.user.approval_status]?.text }}</VChip>
              <VChip :color="props.user.is_active ? 'success' : 'error'" size="small" class="ms-2" label>{{ props.user.is_active ? 'Aktif' : 'Nonaktif' }}</VChip>
            </template>
          </VListItem>
        ...
        <template v-if="quotaDetails">
          <VDivider class="my-4" />
          <div class="text-overline mb-2">Informasi Kuota & Akses</div>
          ...
          <VAlert v-if="quotaDetails.isUnlimited" variant="tonal" color="success" icon="tabler-infinity" class="mb-4" title="Langganan Unlimited Aktif">
              ...
              <div v-if="quotaDetails.expiryDate" class="text-caption mt-1">Berlaku hingga: {{ quotaDetails.expiryDate }} (sisa {{ quotaDetails.remainingDays }} hari)</div>
          </VAlert>
          
          <div v-else-if="quotaDetails.totalQuotaMB > 0">
            <VRow>
              <VCol cols="12" sm="4"><VListItem prepend-icon="tabler-database" title="Total Kuota" :subtitle="formatDataSize(quotaDetails.totalQuotaMB)" class="pa-0" /></VCol>
              <VCol cols="12" sm="4"><VListItem prepend-icon="tabler-database-import" title="Terpakai" :subtitle="formatDataSize(quotaDetails.usedQuotaMB)" class="pa-0" /></VCol>
              <VCol cols="12" sm="4"><VListItem prepend-icon="tabler-database-export" title="Sisa Kuota" :subtitle="formatDataSize(quotaDetails.remainingQuotaMB)" class="pa-0" subtitle-class="font-weight-bold" /></VCol>
            </VRow>
        ...
        <div v-else-if="user.role === 'ADMIN' || user.role === 'SUPER_ADMIN'">
           <VDivider class="my-4" />
           <div class="text-overline mb-2">Informasi Kuota & Akses</div>
           <VAlert variant="tonal" color="success" icon="tabler-shield-check" density="compact">Akses internet untuk peran ini bersifat <strong>Unlimited</strong> dan tidak dibatasi kuota.</VAlert>
        </div>
        
        <VDivider class="my-4" />
        <div class="text-overline mb-2">Informasi Waktu</div>
      ...
      <VCardActions class="pa-4 d-flex justify-end">
        <VBtn variant="tonal" color="secondary" @click="onClose">Tutup</VBtn>
      </VCardActions>
    </VCard>
  </VDialog>
</template>

<style scoped> .v-list-item { padding-inline: 4px !important; } </style>