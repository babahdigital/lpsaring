...
async function setupMissingProfiles() {
  ...
  // --- [PERBAIKAN UTAMA] ---
  // Ambil nama profil dari konfigurasi, bukan di-hardcode.
  const profileAktif = config.public.profileAktifName
  const profileUnlimited = config.public.profileUnlimitedName

  // Validasi jika konfigurasi di .env belum diatur
  if (!profileAktif || !profileUnlimited) {
    snackbar.add({ type: 'error', title: 'Konfigurasi Error', text: 'Nama profil wajib belum diatur di file .env frontend.' })
    loading.value = false
    return
  }

  const requiredProfiles = [profileAktif, profileUnlimited]
    ...
    for (const profileName of profilesToCreate) {
      await $api('/admin/profiles', {
        ...
        body: {
          ...
          description: `Profil sistem otomatis untuk paket ${profileName === profileUnlimited ? 'tanpa batas kuota' : 'berkuota'}.`,
        },
      })
    }
