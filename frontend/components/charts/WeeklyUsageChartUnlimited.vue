...
const quotaWeeklyBarOptions = computed<ApexOptions>(() => {
  ...
  const barColors = processedSeriesData.map((value: unknown, index) => {
    const numValue = Number(value) || 0
    if (index === processedSeriesData.length - 1 && numValue > 0 && !allZero) {
  ...
  return {
    ...
    tooltip: {
      ...
      custom({ series, seriesIndex, dataPointIndex, w }: {
        ...
        const usageValue = series[seriesIndex]?.[dataPointIndex] ?? 0
...
function updateChartData() {
  ...
  else if (newWeeklyData?.success && Array.isArray(newWeeklyData.data)) {
    const rawSeriesData = newWeeklyData.data.slice(-7)
    while (rawSeriesData.length < 7) {
      rawSeriesData.unshift(0)
    }
    newSeriesDataValues = rawSeriesData.map((val: any) => Number(Number.parseFloat(val?.usage_mb?.toString() ?? '0').toFixed(1)) || 0)

    if (newSeriesDataValues.every((d: any) => (d?.usage_mb ?? 0) === 0)) {
                  ...
                  <div ref="chartContainerActualRef" class="weekly-chart-container-actual" :style="{ minHeight: chartHeightInPx, flexGrow: 1, position: 'relative' }">
                    <div class="chart-inner-wrapper">
                      ...
                      <div v-else-if="!props.parentLoading" class="chart-fallback-container text-center pa-2 d-flex flex-column justify-center align-items-center" :style="{ height: '100%', width: '100%', minHeight: chartHeightInPx }">
                        ...
                        <VIcon v-else-if="!isLoadingInternalProcessing && (props.weeklyUsageData == null || props.weeklyUsageData.success !== true || (props.weeklyUsageData.data != null && props.weeklyUsageData.data.every((d: any) => (d?.usage_mb ?? 0) === 0)))" size="32" :color="infoDisplayColor" class="mb-1">
                ...
                <VCol cols="12" sm="6" class="pa-2 text-center">
                  ...
                  <div class="summary-item-content d-flex flex-column align-center">
                    <h6 class="font-weight-medium summary-value">
                      {{ (weeklyUsageData?.data && weeklyUsageData.data.length > 0) ? formatQuota(weeklyUsageData.data[weeklyUsageData.data.length - 1] ?? 0) : 'N/A' }}
                    </h6>
                  </div>
                </VCol>
              </VRow>
