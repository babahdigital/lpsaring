...
const setPrimaryColor = useDebounceFn((color: { main: string, darken: string }) => {
  const currentTheme = vuetifyTheme.themes.value?.[vuetifyTheme.name.value]
  if (currentTheme?.colors) {
    currentTheme.colors.primary = color.main
    currentTheme.colors['primary-darken-1'] = color.darken
  }
...
async function resetCustomizer() {
  if (isCookieHasAnyValue.value) {
    ...
    if (vuetifyTheme.themes.value?.light?.colors) {
    ...
    if (vuetifyTheme.themes.value?.dark?.colors) {
