...
export function useCookie<T = string | null | undefined>(name: string, _opts?: CookieOptions<T>): CookieRef<T> {
  ...
  watch(cookie, () => {
    const { decode, encode, default: _default, watch: _watch, ...serializeOptions } = opts
    document.cookie = serializeCookie(name, cookie.value, serializeOptions)
  })

  return cookie as CookieRef<T>
}

function serializeCookie(name: string, value: any, opts: _CookieOptions = {}) {
